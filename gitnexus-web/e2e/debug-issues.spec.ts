...
debugTest(
	"debug: process view Reset View button",
	async ({ page }, testInfo) => {
		...
		await expect(page.getByText(/\d+ processes detected/)).toBeVisible({
			timeout: 10_000,
		});
		...
		await page.screenshot({
			path: testInfo.outputPath("debug-modal-open.png"),
			fullPage: true,
		});
		...
		await page.screenshot({
			path: testInfo.outputPath("debug-modal-zoomed.png"),
			fullPage: true,
		});
...
debugTest(
	"debug: lightbulb clears node selection dimming",
	async ({ page }, testInfo) => {
		await connectToServer(page);
		...
		await page.screenshot({
			path: testInfo.outputPath("debug-before-select.png"),
			fullPage: true,
		});
		...
		await page.screenshot({
			path: testInfo.outputPath("debug-node-selected.png"),
			fullPage: true,
		});

		// Check the lightbulb button state
		const lightbulbBtn = page.locator(
			'button[title*="Turn off"], button[title*="Turn on"]',
		);
		...
		await page.screenshot({
			path: testInfo.outputPath("debug-after-lightbulb.png"),
			fullPage: true,
		});
