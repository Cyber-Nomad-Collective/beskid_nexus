		...
		return (
			<div className="flex h-full flex-col items-center justify-center p-6 text-center">
				...
				<p className="max-w-xs text-sm text-muted-foreground">
					Processes are execution flows traced from entry points. Load a codebase to
					see detected processes.
				</p>
			</div>
		);
	}
							...
							<div className="pb-2">
								{filteredProcesses.cross.map((process) => (
									<ProcessItem
										...
										onView={() =>
											handleViewProcess(process.id, process.label, "cross_community")
										}
							...
							<div className="pb-2">
								{filteredProcesses.intra.map((process) => (
									<ProcessItem
										...
										isFocused={focusedProcessId === process.id}
										onView={() =>
											handleViewProcess(process.id, process.label, "intra_community")
										}
										onToggleFocus={() => handleToggleFocusForProcess(process.id)}
									/>
								))}
