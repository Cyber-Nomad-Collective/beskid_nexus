    ...
    <div
      className={`overflow-hidden rounded-xl border transition-all duration-300 ${
        isActive
          ...
          : 'border-border hover:border-primary/20 hover:shadow-glow-soft'
      } `}
    >
      {/* Title bar */}
      <div className="flex items-center gap-2 border-b border-border bg-muted px-4 py-2.5">
        ...
        <span className="flex-1 text-center font-mono text-[11px] text-muted-foreground">{label}</span>
        <CopyButton text={command} />
      </div>
      {/* Command body */}
      <div className="flex items-center gap-3 bg-background px-4 py-3.5 font-mono text-sm">
        <span className="text-primary/60 select-none" aria-hidden="true">
          $
        </span>
        <code className="flex-1 overflow-x-auto tracking-wide whitespace-nowrap text-foreground">
          {command}
        </code>
      </div>
    </div>
...
function StepDot({ state, number }: { state: StepState; number: number }) {
  ...
  if (state === 'active') {
    return (
      <div className="relative flex h-6 w-6 shrink-0 items-center justify-center">
        ...
        <div className="flex h-6 w-6 items-center justify-center rounded-full border border-primary/60 bg-primary/20">
          <span className="text-[10px] leading-none font-semibold text-primary">{number}</span>
        </div>
      </div>
    );
  }
  return (
    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-border bg-muted">
      <span className="text-[10px] leading-none font-semibold text-muted-foreground">{number}</span>
    </div>
  );
}
...
function PollingBar() {
  return (
    <div
      className="flex animate-fade-in items-center gap-3 rounded-xl border border-primary/15 bg-primary/5 px-4 py-3"
      aria-live="polite"
      role="status"
    >
      <div className="relative shrink-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="h-5 w-5 animate-pulse rounded-full border border-primary/25" />
        </div>
      </div>
      ...
      <div className="min-w-0 flex-1">
        <p className="text-xs font-medium text-muted-foreground">
          ...
          <span className="ml-0.5 inline-flex text-muted-foreground">
            <span className="animate-pulse">...</span>
          </span>
        </p>
        <p className="mt-0.5 text-[11px] text-muted-foreground">Will auto-connect when detected</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative animate-fade-in overflow-hidden rounded-3xl border border-border bg-card p-7">
      ...
      <div className="relative space-y-5">
        ...
        <div
          className="pointer-events-none absolute top-6 bottom-6 left-[11px] w-px bg-border"
          aria-hidden="true"
        />
