...
export function parseTypedDocLink(kind: string, body: string): TypedDocLink | null {
	if (!['spec', 'book', 'nexus', 'bug'].includes(kind)) return null;
	const values = fields(body);
	const ref = values.ref ?? values.id ?? values.slug;
	if (!ref) return null;
	return { kind: kind as TypedDocLinkKind, ref, title: values.title ?? values.label ?? ref };
}
...
export function typedDocHref(link: TypedDocLink): string {
	...
	if (link.kind === 'book') return `https://beskid-lang.org/book/${link.ref.replace(/^\/+|\/+$/g, '')}/`;
	if (link.kind === 'nexus') return `https://nexus.beskid-lang.org/${link.ref.replace(/^\/+/, '')}`;
...
export function TypedDocLinkCard({ link }: { link: TypedDocLink }) {
	return (
		<a href={typedDocHref(link)} target="_blank" rel="noopener noreferrer"
			...
			data-doc-link={link.kind} data-doc-ref={link.ref}>
			<span className="block text-[10px] font-semibold tracking-wide text-muted-foreground uppercase">{link.kind}</span>
			<span className="text-sm font-medium">{link.title}</span>
			<code className="mt-1 block text-[10px] text-muted-foreground">{link.ref}</code>
		</a>
	);
}
