...
const NEXUS_SINGLE_REPO = import.meta.env.VITE_NEXUS_SINGLE_REPO === '1';
const NEXUS_DEFAULT_REPO = import.meta.env.VITE_NEXUS_DEFAULT_REPO || 'compiler';
...
const AppContent = () => {
  ...
  // Beskid Nexus: auto-connect to same-origin backend with baked compiler index
  ...
  useEffect(() => {
    ...
    if (!NEXUS_SINGLE_REPO && !serverUrlParam && !projectParam) return;
    ...
    const targetProject =
      projectParam || (NEXUS_SINGLE_REPO ? NEXUS_DEFAULT_REPO : undefined);
    ...
    tryConnect()
      ...
      .catch((err) => {
        ...
        setTimeout(() => {
          setViewMode(NEXUS_SINGLE_REPO ? 'loading' : 'onboarding');
          setProgress(null);
  ...
  if (NEXUS_SINGLE_REPO && viewMode === 'loading' && progress) {
    return <LoadingOverlay progress={progress} />;
  }

  // Render based on view mode
  if (viewMode === 'onboarding' && !NEXUS_SINGLE_REPO) {
    return (
      <DropZone
