...
describe("readOnly guard – write-operation detection", () => {
	...
	describe("handles tricky cases", () => {
		...
		it('allows "delete" inside a quoted string value', () => {
			expect(
				isWriteQuery('MATCH (n) WHERE n.name CONTAINS "delete" RETURN n'),
			).toBe(false);
		});

		it('allows "CREATE" inside single-quoted string', () => {
			expect(isWriteQuery("MATCH (n) WHERE n.name = 'CREATE_USER' RETURN n")).toBe(
				false,
			);
		});
		...
		it('does not match partial keywords like "CREATED" or "SETTING"', () => {
			expect(isWriteQuery('MATCH (n) WHERE n.status = "CREATED" RETURN n')).toBe(
				false,
			);
			expect(isWriteQuery('MATCH (n) WHERE n.label = "SETTING" RETURN n')).toBe(
				false,
			);
		});
