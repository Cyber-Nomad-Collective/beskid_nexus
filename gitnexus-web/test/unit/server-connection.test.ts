...
describe("normalizeServerUrl", () => {
	...
	it("strips trailing slashes", () => {
		expect(normalizeServerUrl("http://localhost:4747/")).toBe(
			"http://localhost:4747",
		);
		expect(normalizeServerUrl("http://localhost:4747///")).toBe(
			"http://localhost:4747",
	...
	it("strips /api suffix (base URL only)", () => {
		expect(normalizeServerUrl("http://localhost:4747/api")).toBe(
			"http://localhost:4747",
		);
	});

	it("trims whitespace", () => {
		expect(normalizeServerUrl("  localhost:4747  ")).toBe(
			"http://localhost:4747",
		);
	});
...
describe("fetchGraph", () => {
	...
	it("throws backend errors emitted in the NDJSON stream", async () => {
		...
		const stream = new ReadableStream<Uint8Array>({
			start(controller) {
				controller.enqueue(
					encoder.encode('{"type":"error","error":"stream failed"}\n'),
				);
				controller.close();
			},
		});
...
describe("validateBackendUrl", () => {
	...
	it("allows https:// URLs", () => {
		expect(() =>
			validateBackendUrl("https://gitnexus.example.com"),
		).not.toThrow();
		expect(() =>
			validateBackendUrl("https://my-server.internal:4747"),
		).not.toThrow();
	});
	...
	it("rejects non-http schemes", () => {
		...
		expect(() => validateBackendUrl("file:///etc/passwd")).toThrow(
			"must use http:// or https://",
		);
...
describe("setBackendUrl", () => {
	...
	it("rejects non-http/https schemes", () => {
		...
		expect(() => setBackendUrl("file:///etc/passwd")).toThrow(
			"must use http:// or https://",
		);
	});
