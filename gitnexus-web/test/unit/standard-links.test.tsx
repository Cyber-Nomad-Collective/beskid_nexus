...
describe('StandardLinks', () => {
	it('renders typed stable standard metadata with an accessible canonical link', () => {
		...
		expect(link).toHaveAttribute('href', '/standard/tooling--nexus#mcp-transport');
		expect(link).toHaveAttribute('data-standard-link', 'spec');
		expect(link).toHaveAttribute('data-standard-id', 'standard.tooling.nexus.mcp');
		expect(link).toHaveAttribute('data-standard-revision', 'revision-2');
	});

	it('renders persisted version-1 links as spec links', () => {
		render(<StandardLinks links={[{ title: 'Legacy standard link', href: '/platform-spec/a' }]} />);
		expect(screen.getByRole('link', { name: /Legacy standard link/i })).toHaveAttribute(
			'data-standard-link',
			'spec',
		);
	});
