...
describe("init lock — single-process ownership contract", () => {
	itLbugReopen(
		"acquireInitLock succeeds when parent directory does not exist yet",
		async () => {
			const tmp = await createTempDir("gitnexus-lbug-orphan-");
	...
	itLbugReopen(
		"acquireInitLock creates and releases lock file atomically",
		async () => {
			const tmp = await createTempDir("gitnexus-lbug-orphan-");
			const dbPath = path.join(tmp.dbPath, "lbug");
			const lockPath = `${dbPath}.init.lock`;
	...
	itLbugReopen(
		"acquireInitLock blocks concurrent acquire from same process",
		async () => {
			const tmp = await createTempDir("gitnexus-lbug-orphan-");
			const dbPath = path.join(tmp.dbPath, "lbug");
	...
	itLbugReopen(
		"acquireInitLock reclaims stale lock from dead process",
		async () => {
			const tmp = await createTempDir("gitnexus-lbug-orphan-");
			const dbPath = path.join(tmp.dbPath, "lbug");
			const lockPath = `${dbPath}.init.lock`;
