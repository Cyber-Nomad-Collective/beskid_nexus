...
describe("Tree-sitter multi-language parsing", () => {
	...
	describe("Dart", () => {
		const dartQueries = () =>
			getProvider(SupportedLanguages.Dart).treeSitterQueries;
		...
		it("filters private symbols via underscore convention", async () => {
			...
			const defs = extractDefinitions(matches);
			const { dartExportChecker } = await import(
				"../../src/core/ingestion/export-detection.js"
			);
		...
		it("extracts re-exports", async () => {
			...
			const { matches } = parseAndQuery(
				parser,
				readFixture("dart-advanced.dart"),
				dartQueries(),
			);
		...
		it("extracts heritage (extends, implements, with)", async () => {
			...
			for (const match of matches) {
				...
				if (captures["heritage.trait"]) {
					heritage.push({
						class: captures["heritage.class"],
						parent: captures["heritage.trait"],
					});
				}
			}
		...
		it("detects Flutter framework from file paths", async () => {
			...
			expect(detectFrameworkFromPath("lib/screens/home.dart")?.framework).toBe(
				"flutter",
			);
			expect(detectFrameworkFromPath("lib/pages/login.dart")?.framework).toBe(
				"flutter",
			);
			expect(detectFrameworkFromPath("lib/widgets/button.dart")?.framework).toBe(
				"flutter",
			);
			expect(detectFrameworkFromPath("lib/bloc/user_bloc.dart")?.framework).toBe(
				"flutter",
			);
			expect(detectFrameworkFromPath("lib/services/api.dart")?.framework).toBe(
				"flutter",
			);
			expect(
				detectFrameworkFromPath("lib/routes/app_router.dart")?.framework,
			).toBe("flutter");

			// main.dart gets highest boost
			expect(detectFrameworkFromPath("lib/main.dart")?.entryPointMultiplier).toBe(
				3.0,
			);
			// widgets get lowest
			expect(
				detectFrameworkFromPath("lib/widgets/button.dart")?.entryPointMultiplier,
			).toBe(1.5);
		});
	});
