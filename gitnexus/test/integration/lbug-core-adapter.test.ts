...
withTestLbugDB(
	...
	(handle) => {
		describe("core adapter", () => {
			it("loadGraphToLbug: loads a minimal graph and node counts match", async () => {
				const { executeQuery: coreExecuteQuery } = await import(
			...
			it("loadFTSExtension(conn): loads on an explicit connection and returns true", async () => {
				...
				const { loadFTSExtension, getDatabase } = await import(
					"../../src/core/lbug/lbug-adapter.js"
				);
			...
			it("getLbugStats: returns correct node and edge counts for seeded data", async () => {
				const { getLbugStats } = await import(
					"../../src/core/lbug/lbug-adapter.js"
				);
			...
			describe("error handling", () => {
				...
				it("ensureFTSIndex is idempotent and caches across writable calls (#1224)", async () => {
					const { ensureFTSIndex } = await import(
						"../../src/core/lbug/lbug-adapter.js"
					);
				...
				it("deleteNodesForFile with non-existent path returns zero deleted", async () => {
					const { deleteNodesForFile } = await import(
						"../../src/core/lbug/lbug-adapter.js"
					);
			...
			itLbugReopen(
				...
				async () => {
					...
					await expect(
						adapter.queryFTS("Function", indexName, "main", 5),
					).resolves.toEqual(
	...
	},
	{
		afterSetup: async (handle) => {
			...
			const { loadGraphToLbug } = await import(
				"../../src/core/lbug/lbug-adapter.js"
			);
