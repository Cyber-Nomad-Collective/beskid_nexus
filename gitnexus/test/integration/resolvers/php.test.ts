...
describe("PHP variadic arity minimum (U1)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-variadic-arity-minimum"),
			() => {},
		);
	}, 60000);
...
describe("PHP transitive trait MRO (U2)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-transitive-traits"),
			() => {},
		);
	}, 60000);
...
describe("PHP parent:: bypasses traits (U3)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-parent-vs-trait"),
			() => {},
		);
	}, 60000);
	...
	const callsFromTo = (source: string, target: string, file: string) =>
		getRelationships(result, "CALLS").filter(
			(c) =>
				c.source === source && c.target === target && c.targetFilePath === file,
		);
	...
	it("parent::record() resolves to Base::record, NOT Auditable::record", () => {
		...
		expect(
			callsFromTo("callViaParent", "record", "app/Auditable.php").length,
		).toBe(0);
	});

	it("$this->record() still resolves to Auditable::record (trait shadows parent)", () => {
		expect(callsFromTo("callViaThis", "record", "app/Auditable.php").length).toBe(
			1,
		);
...
describe("PHP namespace-aware free-call fallback (U4)", () => {
	...
	it("resolves same-namespace free call (caller in App\\Utils → App\\Utils\\format)", () => {
		expect(
			callsFromTo("callSameNamespace", "format", "src/App/Utils/Format.php")
				.length,
		).toBe(1);
	});
...
describe("PHP method enrichment", () => {
	...
	it("resolves $dog->speak() CALLS edge", () => {
		...
		const speakCall = calls.find(
			(c) => c.target === "speak" && c.sourceFilePath?.includes("app"),
		);
...
describe("PHP overload dispatch", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-overload-dispatch"),
			() => {},
		);
	}, 60000);
	...
	it('resolves format_text_padded("hi", 20) CALLS edge', () => {
		...
		const paddedCall = calls.find(
			(c) =>
				c.target === "format_text_padded" && c.sourceFilePath?.includes("app"),
		);
...
describe("PHP abstract dispatch", () => {
	...
	it("marks interface Repository.find as isAbstract (conditional)", () => {
		...
		const baseFind = methods.find(
			(n) =>
				n.name === "find" &&
				n.properties.filePath?.includes("Contracts/Repository"),
		);
	...
	it("resolves $repo->find(42) CALLS edge", () => {
		...
		const findCall = calls.find(
			(c) => c.target === "find" && c.sourceFilePath?.includes("app"),
		);
	...
	it("resolves $repo->save($user) CALLS edge", () => {
		const calls = getRelationships(result, "CALLS");
		const saveCall = calls.find(
			(c) => c.target === "save" && c.sourceFilePath?.includes("app"),
		);
		expect(saveCall).toBeDefined();
	});
...
describe("PHP Child extends ParentClass — inherited method resolution (SM-9)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-child-extends-parent"),
			() => {},
		);
	}, 60000);
...
describe("PHP fully-qualified type-hint resolution (Codex #1497)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-fqn-cross-namespace"),
			() => {},
		);
	}, 60000);
	...
	it("detects both User classes in distinct namespaces", () => {
		...
		const userClasses = getNodesByLabelFull(result, "Class").filter(
			(n) => n.name === "User",
		);
		expect(userClasses.length).toBe(2);
		const userFiles = userClasses
			.map((c) => c.properties.filePath as string)
			.sort();
		expect(
			userFiles.some(
				(f) => f.includes("Models/User.php") || f.includes("Models\\User.php"),
			),
		).toBe(true);
		expect(
			userFiles.some(
				(f) => f.includes("Other/User.php") || f.includes("Other\\User.php"),
			),
		).toBe(true);
	});
	...
	it("simple-name `User $u` parameter resolves to the imported App\\Models\\User (control case)", () => {
		...
		expect(callsFromTo("saveLocal", "record", "app/Models/User.php").length).toBe(
			1,
		);
		expect(callsFromTo("saveLocal", "record", "app/Other/User.php").length).toBe(
			0,
		);
	});
});
...
describe("PHP MRO arity-mismatch fallthrough", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-mro-arity-mismatch"),
			() => {},
		);
	}, 60000);
	...
	const callsFromTo = (source: string, target: string, targetFilePath: string) =>
		getRelationships(result, "CALLS").filter(
			(c) =>
				c.source === source &&
				c.target === target &&
				c.targetFilePath === targetFilePath,
		);
	...
	it("arity-incompatible most-derived override does NOT fall through to ParentModel::method", () => {
		...
		expect(
			callsFromTo("callIncompatible", "method", "app/Models/ParentModel.php")
				.length,
		).toBe(0);
		expect(
			callsFromTo("callIncompatible", "method", "app/Models/ChildModel.php")
				.length,
		).toBe(0);
	});
	...
	it("arity-compatible most-derived override emits exactly one CALLS edge to ChildModel::compat", () => {
		...
		expect(
			callsFromTo("callCompatible", "compat", "app/Models/ChildModel.php").length,
		).toBe(1);
		expect(
			callsFromTo("callCompatible", "compat", "app/Models/ParentModel.php").length,
		).toBe(0);
	});
	...
	it("arity-incompatible class with no parent emits zero CALLS edges (regression check)", () => {
		...
		expect(
			callsFromTo("callNoParent", "method", "app/Models/Orphan.php").length,
		).toBe(0);
	});
	...
	it("arity-compatible most-derived call still resolves to ChildModel::method (happy path)", () => {
		...
		expect(
			callsFromTo("callMostDerivedHappy", "method", "app/Models/ChildModel.php")
				.length,
		).toBe(1);
		expect(
			callsFromTo("callMostDerivedHappy", "method", "app/Models/ParentModel.php")
				.length,
		).toBe(0);
	});
});
...
describe("PHP typed-property double-match dedup", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-typed-property-dedup"),
			() => {},
		);
	}, 60000);
	...
	it("emits zero stray Variable defs for typed property and promoted typed parameter", () => {
		...
		expect(
			variables.filter((n) => n === "$promotedRepo" || n === "promotedRepo")
				.length,
		).toBe(0);
	});
...
describe("PHP dynamic dispatch — negative regression suite", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "php-dynamic-calls"),
			() => {},
		);
	}, 60000);
	...
	it("dynamic property read ($obj->$prop) emits no read-edge to dynamicProp", () => {
		...
		const reads = getRelationships(result, "READS").filter(
			(r) =>
				r.target === "dynamicProp" &&
				r.sourceFilePath === "app/Services/Dynamic.php",
		);
...
describe("PHP unresolved-receiver fallback exact-required-arity gate", () => {
	...
	const fallbackEdgeFromTo = (source: string, target: string) =>
		getRelationships(result, "CALLS").filter(
			(c) =>
				c.source === source &&
				c.target === target &&
				c.targetFilePath === "app/Models/Handler.php",
		);
	...
	it("argCount === required (1===1) on candidate with default param still emits edge", () => {
		expect(
			fallbackEdgeFromTo("callDefaultExactRequired", "withDefault").length,
		).toBe(1);
	});
	...
	it("argCount > required (2>1) on candidate with default param emits NO edge post-fix", () => {
		...
		expect(
			fallbackEdgeFromTo("callDefaultBeyondRequired", "withDefault").length,
		).toBe(0);
	});

	it("variadic candidate, argCount === required (1===1) emits edge", () => {
		expect(
			fallbackEdgeFromTo("callVariadicAtRequired", "variadicLog").length,
		).toBe(1);
	});

	it("variadic candidate, argCount > required (2>1) emits edge (relaxed)", () => {
		expect(
			fallbackEdgeFromTo("callVariadicBeyondRequired", "variadicLog").length,
		).toBe(1);
	});

	it("variadic candidate, argCount < required (1<2) emits NO edge", () => {
		expect(
			fallbackEdgeFromTo("callVariadicBelowRequired", "variadicLogTwoRequired")
				.length,
		).toBe(0);
	});
});
