...
describe("Ruby qualified class names", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "ruby-qualified-types"),
			() => {},
		);
	}, 60000);
...
describe("Ruby method enrichment (visibility, isStatic, parameters)", () => {
	...
	it("emits HAS_METHOD edges for Animal and Dog", () => {
		...
		expect(
			hasMethod.find((e) => e.source === "Animal" && e.target === "speak"),
		).toBeDefined();
		expect(
			hasMethod.find((e) => e.source === "Animal" && e.target === "classify"),
		).toBeDefined();
		expect(
		...
		expect(
			hasMethod.find(
				(e) => e.source === "Animal" && e.target === "internal_state",
			),
		).toBeDefined();
		// Dog has speak, energy_level
		expect(
			hasMethod.find((e) => e.source === "Dog" && e.target === "speak"),
		).toBeDefined();
		expect(
			hasMethod.find((e) => e.source === "Dog" && e.target === "energy_level"),
		).toBeDefined();
	});
	...
	it("marks internal_state as private (when enriched)", () => {
		...
		const internalState = methods.find(
			(m) =>
				m.name === "internal_state" && m.properties.filePath?.includes("animal"),
		);
	...
	it("resolves dog.speak member call from main to Dog#speak", () => {
		...
		const speakCall = calls.find(
			(c) =>
				c.source === "main" &&
				c.target === "speak" &&
				c.targetFilePath.includes("animal"),
		);
...
describe("Ruby overload dispatch (format vs format_with_prefix)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "ruby-overload-dispatch"),
			() => {},
		);
	}, 60000);
	...
	it("emits HAS_METHOD edges for both methods on Formatter", () => {
		const hasMethod = getRelationships(result, "HAS_METHOD");
		expect(
			hasMethod.find((e) => e.source === "Formatter" && e.target === "format"),
		).toBeDefined();
		expect(
			hasMethod.find(
...
describe("Ruby Child extends Parent — inherited method resolution (SM-9)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "ruby-child-extends-parent"),
			() => {},
		);
	}, 60000);
