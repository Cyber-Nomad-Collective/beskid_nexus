...
describe("Go same-package factory return type inference", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "go-same-package-factory"),
			() => {},
		);
	}, 60000);
...
describe("Go aliased package selector resolution", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "go-aliased-package-import"),
			() => {},
		);
	}, 60000);
	...
	it("resolves util.Log() through an aliased package import", () => {
		...
		const logCall = calls.find(
			(c) =>
				c.target === "Log" &&
				c.source === "main" &&
				c.targetFilePath === "internal/util/log.go",
		);
		expect(logCall).toBeDefined();
	});
});
...
describe("Go method owner resolution across package files", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "go-split-method-owner"),
			() => {},
		);
	}, 60000);
...
describe("Go method enrichment", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "go-method-enrichment"),
			() => {},
		);
	}, 60000);
...
describe("Go Child embeds Parent — inherited method resolution (SM-9)", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "go-child-extends-parent"),
			() => {},
		);
	}, 60000);
