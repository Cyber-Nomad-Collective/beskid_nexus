...
describe("TypeScript HOC-wrapped variable declarations", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "typescript-hoc-wrapped"),
			() => {},
		);
	}, 60000);
	...
	it("useCallback: handleClick → doStuff and handleClick → fmt", () => {
		...
		const calls = getRelationships(result, "CALLS").filter(
			(c) =>
				c.sourceFilePath === "src/use-callback.ts" && c.source === "handleClick",
		);
		const targets = new Set(calls.map((c) => c.target));
	...
	it("useCallback: handleSubmit → doStuff (sibling const, separate caller)", () => {
		...
		expect(
			fromHandleSubmit.length,
			"handleSubmit must call doStuff",
		).toBeGreaterThan(0);
	});
	...
	it("debounce: debouncedSearch → doStuff (utility-HOC form)", () => {
		...
		const calls = getRelationships(result, "CALLS").filter(
			(c) =>
				c.sourceFilePath === "src/debounce.ts" && c.source === "debouncedSearch",
		);
	...
	it("each HOC-wrapped const registers as a Function node", () => {
		...
		expect(functions, "debounce-wrapped debouncedSearch").toContain(
			"debouncedSearch",
		);
	});
	...
	it("bare statement-level HOC calls do not produce phantom Functions", () => {
		...
		// names are absent.
		expect(sources, "no phantom useCallback as caller").not.toContain(
			"useCallback",
		);
		...
		expect(
			fromFile.length,
			"unbound HOC inner calls source from File",
		).toBeGreaterThan(0);
	});
	...
	it("no phantom self-loops in HOC-wrapped fixtures", () => {
		...
		expect(
			buttonCalls,
			"Button must NOT call forwardRef (phantom self-loop)",
		).toEqual([]);
		...
		expect(
			handleClickCalls,
			"handleClick must NOT call useCallback (phantom self-loop)",
		).toEqual([]);
	});
	...
	it("handleClick and handleSubmit do not cross-attribute (no first-sibling-wins)", () => {
		...
		const fromHandleSubmit = calls.filter((c) => c.source === "handleSubmit");
		expect(
			fromHandleClick.length,
			"handleClick must call doStuff",
		).toBeGreaterThan(0);
		expect(
			fromHandleSubmit.length,
			"handleSubmit must call doStuff",
		...
		const stray = calls.filter(
			(c) => c.source !== "handleClick" && c.source !== "handleSubmit",
		);
	...
	it("nested HOCs (memo(forwardRef(...))): Wrapped is NOT a Function (known limitation)", () => {
		...
		expect(
			functions,
			"Wrapped (nested HOC) must NOT be a Function node",
	...
	it("nested HOCs: helper() call inside the deepest arrow does NOT source from Function:Wrapped", () => {
		...
		expect(
			helperCalls.length,
			"helper call must still be captured",
		).toBeGreaterThan(0);
		...
		const functionSourced = allNestedCalls.filter(
			(c) => c.sourceLabel === "Function",
		);
		expect(
			functionSourced,
