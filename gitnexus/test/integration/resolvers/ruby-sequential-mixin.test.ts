...
describe("Ruby mixin heritage: sequential vs worker parity", () => {
	...
	it("labels Ruby modules as Trait in both modes", () => {
		...
		// are emitted.
		expect(
			getNodesByLabel(sequential, "Module").filter((n) => n !== "lib"),
		).toEqual([]);
	...
	it("sequential mode resolves prepend-only method: call_prepended_marker → PrependedOverride#prepended_marker", () => {
		...
		const owners = resolvedMethodOwners(
			sequential,
	...
	it("worker mode resolves the same include and prepend-only targets", () => {
		...
		expect(resolvedMethodOwners(workers, "call_greet", "greet")).toContain(
			"Greetable",
		);
		expect(
			resolvedMethodOwners(workers, "call_prepended_marker", "prepended_marker"),
		).toContain("PrependedOverride");
	});
