...
describe.skipIf(!swiftAvailable)(
	"Swift implicit imports (cross-file visibility)",
	() => {
		let result: PipelineResult;
...
describe.skipIf(!swiftAvailable)("Swift extension deduplication", () => {
	...
	it("resolves Product() constructor despite extension creating duplicate class node", () => {
		const calls = getRelationships(result, "CALLS");
		const ctorCall = calls.find(
			(c) => c.target === "Product" && c.source === "process",
		);
		expect(ctorCall).toBeDefined();
	});
	...
	it("resolves product.save() to Product.swift (primary definition)", () => {
		...
		const saveCall = calls.find(
			(c) =>
				c.target === "save" &&
				c.source === "process" &&
				c.targetFilePath === "Product.swift",
		);
		expect(saveCall).toBeDefined();
	});
});
...
describe.skipIf(!swiftAvailable)(
	"Swift constructor call fallback (no new keyword)",
	() => {
		let result: PipelineResult;
...
describe.skipIf(!swiftAvailable)(
	"Swift export visibility (internal vs private)",
	() => {
		let result: PipelineResult;
...
describe.skipIf(!swiftAvailable)(
	"Swift if let / guard let binding resolution",
	() => {
		let result: PipelineResult;
		...
		it("user.save() in if-let does NOT resolve to Repo#save", () => {
			const calls = getRelationships(result, "CALLS");
			const wrongSave = calls.find(
				(c) => c.target === "save" && c.source === "processIfLet",
			);
			if (wrongSave) {
...
describe.skipIf(!swiftAvailable)(
	"Swift await / try expression unwrapping",
	() => {
		let result: PipelineResult;
...
describe.skipIf(!swiftAvailable)(
	"Swift for-in loop element type inference",
	() => {
		let result: PipelineResult;
...
describe.skipIf(!swiftAvailable)("Swift abstract dispatch", () => {
	...
	beforeAll(async () => {
		result = await runPipelineFromRepo(
			path.join(FIXTURES, "swift-abstract-dispatch"),
			() => {},
		);
	}, 60000);
...
describe.skipIf(!swiftAvailable)(
	"Swift overloaded method disambiguation",
	() => {
		let result: PipelineResult;

		beforeAll(async () => {
			result = await runPipelineFromRepo(
				path.join(FIXTURES, "swift-overload-dispatch"),
				() => {},
			);
		}, 60000);
		...
		it("detects 2 distinct find Method nodes on SqlRepository", () => {
			...
			const sqlRepoFinds = methods.filter(
				(m) =>
					m.name === "find" && m.properties.filePath?.includes("SqlRepository"),
			);
