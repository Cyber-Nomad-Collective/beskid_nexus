...
function runCli(command: string, cwd: string, timeoutMs = 15000) {
	return spawnSync(
		process.execPath,
		["--import", tsxImportUrl, cliEntry, command],
		{
			cwd,
			encoding: "utf8",
			timeout: timeoutMs,
...
describe("CLI end-to-end", () => {
	...
	it("already-up-to-date analyze fails when registry entry is missing (#1169)", () => {
		const gnHome = fs.mkdtempSync(
			path.join(os.tmpdir(), "gn-1169-fastpath-home-"),
		);
	...
	describe("analyze --name <alias> and --allow-duplicate-name (#829)", () => {
		...
		it("--name alias stores; collision rejects; --allow-duplicate-name bypasses", () => {
			...
			try {
				...
				expect(afterStep3.every((e: { name: string }) => e.name === "shared")).toBe(
					true,
				);
				...
				const step3Basenames = afterStep3.map((e: { path: string }) =>
				...
				expect(afterStep3.map((e: { path: string }) => e.path)).toContain(
					afterStep1[0].path,
				);
	...
	describe("remove <target> (#664)", () => {
		it("dry-run lists, --force deletes, missing target is a no-op warning", () => {
			...
			try {
				...
				expect(r2Output, "dry-run must surface the repo path").toContain(
					afterIndex[0].path,
				);
				expect(r2Output, "dry-run must surface the storage path").toContain(
					storagePath,
				);
				expect(fs.existsSync(storagePath)).toBe(true);
				...
				expect(r3Output, "success output must surface the alias").toContain(
					"alias-a",
				);
				expect(r3Output, "success output must surface the repo path").toContain(
					afterIndex[0].path,
				);
				expect(r3Output, "success output must surface the storage path").toContain(
					storagePath,
				);
		...
		it("ambiguous target (two entries share alias via --allow-duplicate-name) errors without mutating registry", () => {
			...
			try {
				...
				const repoAEntry = before.find(
					(e: { path: string }) =>
						path.basename(e.path) === "dup" &&
						e.path.includes(path.basename(parentA)),
				);
				expect(
					repoAEntry,
		...
		it("refuses to proceed when a registry entry points storagePath outside <repo>/.gitnexus (#1003)", () => {
			...
			try {
				...
				expect(
					fs.existsSync(path.join(repo, ".gitnexus")),
					"storage dir must survive",
				).toBe(true);
				expect(fs.existsSync(path.join(repo, ".git")), ".git must survive").toBe(
					true,
				);
	...
	describe("clean --all with a poisoned registry entry (#1003)", () => {
		it("skips poisoned entries, cleans valid ones, never deletes the working tree", () => {
			const gnHome = fs.mkdtempSync(
				path.join(os.tmpdir(), "gn-home-clean-poison-"),
			);
			...
			try {
				...
				for (const [repo, alias] of [
					...
					expect(
						r.status,
						`analyze ${alias} exited ${r.status}: ${r.stdout}${r.stderr}`,
					).toBe(0);
				}
				...
				// working tree.
				const poisoned = original.map(
					(e: { name: string; storagePath: string; path: string }) =>
				...
				expect(
					fs.existsSync(repoBad),
					"poisoned repo working tree must survive",
				).toBe(true);
				expect(
					fs.existsSync(path.join(repoBad, ".gitnexus")),
					"poisoned repo .gitnexus must survive (guard refused to rm repo root)",
				).toBe(true);
				expect(fs.existsSync(path.join(repoBad, ".git")), ".git must survive").toBe(
					true,
				);
				...
				expect(fs.existsSync(repoGood), "good repo working tree must survive").toBe(
					true,
				);
