...
describe("ORM dataflow detection", () => {
	...
	it("creates QUERIES edges for Prisma calls", () => {
		...
		const prismaEdges = queryEdges.filter((e) =>
			e.source.includes("prisma-service"),
		);
	...
	it("creates QUERIES edges for Supabase calls", () => {
		...
		const supabaseEdges = queryEdges.filter((e) =>
			e.source.includes("supabase-service"),
		);
	...
	it("creates CodeElement nodes for ORM models", () => {
		...
		result.graph.forEachNode((n) => {
			if (
				n.label === "CodeElement" &&
				n.properties.description?.includes("model/table")
			) {
