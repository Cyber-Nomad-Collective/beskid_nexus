...
function writeTempWorker(
	prefix: string,
	source: string,
): { tempDir: string; workerPath: string } {
...
describe("worker pool integration", () => {
    ...
    `,
		...
		try {
			...
			expect(cap.records().some((r) => r.msg === "warning before result")).toBe(
				true,
			);
		} finally {
			cap.restore();
	...
	it("replaces a timed-out worker and retries with a longer timeout", async () => {
		const tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-worker-retry-"),
		);
    ...
    `,
		...
		try {
			...
			expect(
				cap
					.records()
					.some((r) => String(r.msg ?? "").includes("Retrying with 2s timeout")),
			).toBe(true);
		} finally {
			cap.restore();
			fs.rmSync(tempDir, { recursive: true, force: true });
	...
	it("rejects dispatch when replacement worker crashes during startup", async () => {
		const tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-worker-replace-fail-"),
		);
		const markerPath = path.join(tempDir, "first-attempt.txt");
		const workerPath = path.join(tempDir, "worker.js");
		fs.writeFileSync(
    ...
    `,
		...
		try {
			await expect(
				pool.dispatch<any, any>([{ path: "crash.ts", content: "" }]),
			).rejects.toThrow(/simulated startup crash|exited with code|idle timeout/);
	...
	it("preserves global path order across split-and-retry", async () => {
		const tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-worker-split-"),
		);
    ...
    `,
		...
		try {
			...
			expect(
				cap
					.records()
					.some((r) => String(r.msg ?? "").includes("Splitting into 1/1 item jobs")),
			).toBe(true);
		} finally {
			cap.restore();
			fs.rmSync(tempDir, { recursive: true, force: true });
    ...
    `,
		...
		try {
			await expect(
				pool.dispatch<any, any>([{ path: "stalled.ts", content: "" }]),
			).rejects.toThrow(/sequential fallback/);
		} finally {
			fs.rmSync(tempDir, { recursive: true, force: true });
		}
	});
	...
	it("does not resolve early when a stalled peer job is requeued during another worker finish", async () => {
		const tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-worker-race-"),
		);
		const markerPath = path.join(tempDir, "stalled-once.txt");
		const workerPath = path.join(tempDir, "worker.js");
		fs.writeFileSync(
	...
	it("completes split-and-retry when the timed-out worker is the only active worker", async () => {
		...
		const tempDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-worker-sole-active-"),
		);
		const markerPath = path.join(tempDir, "stalled-once.txt");
		const workerPath = path.join(tempDir, "worker.js");
		fs.writeFileSync(
    ...
    `,
		...
		try {
			...
			expect(
				cap.records().some((r) => String(r.msg ?? "").includes("Splitting into")),
			).toBe(true);
		} finally {
			cap.restore();
			fs.rmSync(tempDir, { recursive: true, force: true });
    ...
    `,
		...
		try {
			await expect(
				pool.dispatch<any, any>([{ path: "bad.ts", content: "" }]),
			).rejects.toThrow(/protocol error/);
			await expect(
				pool.dispatch<any, any>([{ path: "after.ts", content: "" }]),
			).rejects.toThrow(/previous failure.*protocol error/);
		} finally {
			fs.rmSync(tempDir, { recursive: true, force: true });
		}
	});
    ...
    `,
		...
		try {
			...
			expect(results.map((result) => result.paths)).toEqual([
				["a.ts"],
				["b.ts", "c.ts"],
			]);
		} finally {
	...
	it.skipIf(!hasDistWorker)("dispatch with size 0 rejects clearly", async () => {
		...
		try {
			await expect(
				zeroPool.dispatch([{ path: "x.ts", content: "const x = 1;" }]),
