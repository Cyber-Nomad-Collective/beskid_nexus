...
describe("server-side analyze integration", () => {
	...
	it("failed job emits failure event", () => {
		const job = manager.createJob({
			repoUrl: "https://github.com/user/fail-test",
		});
		const events: any[] = [];
	...
	it("git URL name extraction for clone paths", () => {
		expect(extractRepoName("https://github.com/facebook/react.git")).toBe(
			"react",
		);
	...
	it("concurrency: blocks second job while first is active", () => {
		...
		const job1 = freshManager.createJob({
			repoUrl: "https://github.com/user/repo-a",
		});
		freshManager.updateJob(job1.id, { status: "analyzing" });

		// Second different repo should be rejected
		expect(() =>
			freshManager.createJob({ repoUrl: "https://github.com/user/repo-b" }),
		).toThrow(/already in progress/);

		// Same repo should return existing job
		const job1again = freshManager.createJob({
			repoUrl: "https://github.com/user/repo-a",
		});
		...
		const job2 = freshManager.createJob({
			repoUrl: "https://github.com/user/repo-b",
		});
