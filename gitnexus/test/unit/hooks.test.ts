...
const CJS_HOOK_LOCK = path.resolve(
	__dirname,
	"..",
	"..",
	"hooks",
...
function createGlobalRegistry(
	homeDir: string,
	marker: "both" | "registry" | "repos" = "both",
) {
	...
	for (const [label, hookPath] of [
		...
		it(`${label}: hook reclaims a slot held by a dead pid`, () => {
			...
			try {
				...
				if (fs.existsSync(stalePath)) {
					expect(fs.readFileSync(stalePath, "utf-8").trim()).not.toBe(
						String(deadPid),
					);
				}
			} finally {
      ...
      `;
			...
			try {
				...
				const claimedCount = decisions.filter((d) =>
					...
					.filter((f) => {
						try {
							const o = Number.parseInt(
								fs.readFileSync(path.join(lockDir, f), "utf-8").trim(),
								10,
							);
	...
	for (const [label, hookPath] of [
		...
		it.skipIf(process.platform === "win32")(
			...
			() => {
				const markerPath = path.join(
					os.tmpdir(),
					`gitnexus-hook-called-${process.pid}-${label}`,
				);
	...
	() => {
		for (const [label, hookPath] of [
			...
			it(`${label}: skips augment for real node_modules/gitnexus ps line (npx child)`, () => {
				const markerPath = path.join(
					os.tmpdir(),
					`gn-hook-prodps-${process.pid}-${label}`,
				);
				const lbugPath = path.join(gitNexusDir, "lbug");
				fs.writeFileSync(lbugPath, "");
				fs.rmSync(markerPath, { force: true });
			...
			it(`${label}: skips augment for gitnexus serve child`, () => {
				const markerPath = path.join(
					os.tmpdir(),
					`gn-hook-serve-${process.pid}-${label}`,
				);
				const lbugPath = path.join(gitNexusDir, "lbug");
				fs.writeFileSync(lbugPath, "");
				fs.rmSync(markerPath, { force: true });
			...
			it(`${label}: ENOENT lsof → augment still runs (fail-open)`, () => {
				const markerPath = path.join(
					os.tmpdir(),
					`gn-hook-enoent-${process.pid}-${label}`,
				);
				const lbugPath = path.join(gitNexusDir, "lbug");
				fs.writeFileSync(lbugPath, "");
				fs.rmSync(markerPath, { force: true });
			...
			it(`${label}: ETIMEDOUT lsof → augment skipped (fail-closed)`, () => {
				const markerPath = path.join(
					os.tmpdir(),
					`gn-hook-etime-${process.pid}-${label}`,
				);
				const lbugPath = path.join(gitNexusDir, "lbug");
				fs.writeFileSync(lbugPath, "");
				fs.rmSync(markerPath, { force: true });
			...
			it(`${label}: non-GitNexus ps line → augment runs`, () => {
				const markerPath = path.join(
					os.tmpdir(),
					`gn-hook-other-${process.pid}-${label}`,
				);
				const lbugPath = path.join(gitNexusDir, "lbug");
				fs.writeFileSync(lbugPath, "");
				fs.rmSync(markerPath, { force: true });
			...
			it(`${label}: multiple PIDs — skip if any ps line is GitNexus MCP`, () => {
				const markerPath = path.join(
					os.tmpdir(),
					`gn-hook-multi-${process.pid}-${label}`,
				);
				const lbugPath = path.join(gitNexusDir, "lbug");
				fs.writeFileSync(lbugPath, "");
				fs.rmSync(markerPath, { force: true });
			...
			it(`${label}: ps ENOENT → augment runs (ignore that PID)`, () => {
				const markerPath = path.join(
					os.tmpdir(),
					`gn-hook-pseno-${process.pid}-${label}`,
				);
				const lbugPath = path.join(gitNexusDir, "lbug");
				fs.writeFileSync(lbugPath, "");
				fs.rmSync(markerPath, { force: true });
	...
	for (const [label, hookPath] of [
		...
		it(`${label}: PostToolUse emits stale from a linked worktree pointing at an indexed canonical repo`, () => {
			...
			try {
				...
				expect(
					fs.existsSync(path.join(path.dirname(worktreePath), ".gitnexus")),
				).toBe(false);
