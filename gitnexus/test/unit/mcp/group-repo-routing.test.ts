...
vi.mock("../../../src/core/lbug/pool-adapter.js", async (importOriginal) => {
	const actual =
		await importOriginal<
			typeof import("../../../src/core/lbug/pool-adapter.js")
		>();
...
vi.mock("../../../src/mcp/core/lbug-adapter.js", async (importOriginal) => {
	const actual =
		await importOriginal<
			typeof import("../../../src/mcp/core/lbug-adapter.js")
		>();
	return { ...actual, ...lbugMocks };
});
...
`,
		...
		groupSpyContext = vi
			.spyOn(GroupService.prototype, "groupContext")
			.mockResolvedValue({
				group: "g1",
				results: [],
			});
	...
	it("returns error for unknown member path", async () => {
		const backend = new LocalBackend();
		const out = await backend.callTool("query", {
			repo: "@g1/not-a-member",
			query: "x",
		});
		expect(out).toHaveProperty("error");
		expect(String((out as { error: string }).error)).toMatch(
			/Unknown member path/i,
		);
	});
	...
	it("unknown group_* tools mention removal", async () => {
		const backend = new LocalBackend();
		await expect(
			backend.callTool("group_query", { name: "g1", query: "x" }),
		).rejects.toThrow(/Removed tools/);
	});

	it("removed group_contracts mentions migration", async () => {
		const backend = new LocalBackend();
		await expect(
			backend.callTool("group_contracts", { name: "g1" }),
		).rejects.toThrow(/Removed tools/);
	});
	...
	it("removed group_status mentions migration", async () => {
		const backend = new LocalBackend();
		await expect(
			backend.callTool("group_status", { name: "g1" }),
		).rejects.toThrow(/Removed tools/);
	});
...
`,
		...
		it.each([
			...
			async ({ method, params, spy }) => {
				...
				expect(
					typeof callArg.repo === "string"
						? (callArg.repo as string).startsWith("@")
						: false,
				).toBe(false);
			},
		);
