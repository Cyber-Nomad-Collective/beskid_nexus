  ...
  process(repo: Repo) {
	...
	describe("destructured call results", () => {
		...
		it("emits callResult for destructured await call", () => {
			const symbolTable = makeSymbolTable([
				{ name: "fetchData", returnType: "Response" },
			]);
			const tree = parse(
	...
	describe("constructor inference (Tier 1 fallback)", () => {
        ...
        `,
				...
				const defaultBinding = typeEnv.constructorBindings.find(
        ...
        `,
				...
				const typeEnv = buildTypeEnv(tree, "rust");
				const newBinding = typeEnv.constructorBindings.find(
					(b) => b.calleeName === "new",
				);
...
`,
		...
		describe("lookupClassByName regression coverage", () => {
			const makeClassLookupTable = (
				...
				const model = createSemanticModel();
				vi
					.spyOn(model.types, "lookupClassByName")
					.mockImplementation((name: string) => classDefs[name] ?? []);
				return model;
			};
...
`,
				...
				const typeEnv = buildTypeEnv(tree, "ruby", {
					model: makeClassLookupTable({
						UserService: [
							createClassDef("UserService", "Class", "models/user_service.rb"),
						],
					}),
				});
...
`,
				...
				const lookupCallableByName = vi.spyOn(
					model.symbols,
  ...
  public string Save() {
	...
	describe("C++ namespaced constructor binding", () => {
      ...
      `,
			...
			const binding = typeEnv.constructorBindings.find(
				(b) => b.varName === "client",
			);
      ...
      `,
			...
			expect(
				typeEnv.constructorBindings.find((b) => b.varName === "user"),
			).toBeUndefined();
		});
	});
...
`,
	...
	describe("assignment chain propagation (Tier 2, depth-1)", () => {
      ...
      `,
			...
			const scopeKey = [...typeEnv.allScopes().keys()].find((k) =>
  ...
  c.validate();
	...
	describe("end-to-end: real TypeEnv → flush → accumulator → enrichment", () => {
		it("enriches exportedTypeMap with bindings from a real TypeScript file", async () => {
			...
			const { enrichExportedTypeMap, type: _ignore } = (await import(
				"../../src/core/ingestion/binding-accumulator.js"
			)) as typeof import("../../src/core/ingestion/binding-accumulator.js") & {
				type: unknown;
			};
...
`;
			...
			expect(exportedTypeMap.get("src/service.ts")?.get("dbClient")).toBe(
				"Database",
			);
		...
		it("respects Tier 0 priority when the SymbolTable pre-populated the export", async () => {
			const { enrichExportedTypeMap } = await import(
				"../../src/core/ingestion/binding-accumulator.js"
			);
