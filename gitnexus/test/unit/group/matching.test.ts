...
describe("normalizeContractId", () => {
	it("lowercases HTTP method", () => {
		expect(normalizeContractId("http::get::/api/users")).toBe(
			"http::GET::/api/users",
		);
	});
	...
	it("trims and lowercases topic", () => {
		expect(normalizeContractId("topic::  Employee.Hired  ")).toBe(
			"topic::employee.hired",
		);
	});

	it("lowercases lib package coordinates", () => {
		expect(normalizeContractId("lib::@Hr/Common::UserDTO")).toBe(
			"lib::@hr/common::userdto",
		);
	});
});
...
describe("runWildcardMatch", () => {
	it("test_runWildcardMatch_fq_service_match", () => {
		const consumer = makeGrpcContract(
