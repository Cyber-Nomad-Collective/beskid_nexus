...
describe("group-path-utils", () => {
	...
	describe("fileMatchesServicePrefix", () => {
		...
		it("matches exact prefix and descendants", () => {
			expect(fileMatchesServicePrefix("services/auth", "services/auth")).toBe(
				true,
			);
			expect(fileMatchesServicePrefix("services/auth/a.ts", "services/auth")).toBe(
				true,
			);
		});

		it("rejects partial-segment matches", () => {
			expect(fileMatchesServicePrefix("services/aut", "services/auth")).toBe(
				false,
			);
			expect(
				fileMatchesServicePrefix("services/authz/a.ts", "services/auth"),
			).toBe(false);
		});

		it("matches Windows-style file paths against POSIX prefix", () => {
			expect(
				fileMatchesServicePrefix("services\\auth\\a.ts", "services/auth"),
			).toBe(true);
			expect(
				fileMatchesServicePrefix("services\\authz\\a.ts", "services/auth"),
			).toBe(false);
	...
	describe("repoInSubgroup", () => {
		...
		it("normalizes Windows-style paths on both sides", () => {
			expect(repoInSubgroup("app\\backend", "app/backend")).toBe(true);
			expect(repoInSubgroup("app/backend/x", "app\\backend")).toBe(true);
			expect(repoInSubgroup("app\\backend\\sub", "app\\backend", true)).toBe(
				false,
			);
		});
	});
