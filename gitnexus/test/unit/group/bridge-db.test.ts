...
describe("writeBridge + read", () => {
	...
	it("test_writeBridge_returns_report_with_insert_counts", async () => {
		const report = await writeBridge(tmpDir, {
			contracts: [
				makeContract(),
				makeContract({ repo: "frontend", role: "consumer" }),
			],
			crossLinks: [],
			repoSnapshots: { backend: { indexedAt: "2026-01-01", lastCommit: "abc" } },
			missingRepos: [],
	...
	itLbugReopen(
		"test_writeBridge_duplicate_contracts_and_links_are_deduped",
		async () => {
...
describe("retryRename", () => {
	...
	it("retries on EBUSY and eventually succeeds", async () => {
		...
		expect(attempts.every(([s, d]) => s === "/src/a" && d === "/dst/b")).toBe(
			true,
		);
	});
...
describe("findContractNode", () => {
	...
	it("returns null on empty index", () => {
		...
		expect(
			findContractNode(index, "backend", "provider", "uid-1", "src/a.ts", "foo"),
		).toBeNull();
	});
	...
	it("tier 2: falls through to filePath + symbolName when symbolUid is empty", () => {
		...
		expect(
			findContractNode(index, "backend", "provider", "", "src/ctrl.ts", "handler"),
		).toBe("node-B");
	});
	...
	it("prefers tier 1 over tier 2 when both could resolve", () => {
		...
		expect(
			findContractNode(index, "backend", "provider", "uid-1", "src/a.ts", "first"),
		).toBe("tier1-id");
	});
});
