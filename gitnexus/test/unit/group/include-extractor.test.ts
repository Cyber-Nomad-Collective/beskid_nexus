...
int main() { return 0; }`,
	...
	describe("cross-repo matching", () => {
		it("provider and consumer produce matching contractIds", async () => {
			...
			const providerDir = fs.mkdtempSync(
				path.join(os.tmpdir(), "gitnexus-include-provider-"),
			);
			...
			const consumerDir = fs.mkdtempSync(
				path.join(os.tmpdir(), "gitnexus-include-consumer-"),
			);
			...
			try {
				...
				const providerIds = new Set(
					providers.map((p) => normalizeContractId(p.contractId)),
				);
...
int main(){return 0;}`;
	...
	describe("finding #6: meta.source reflects extraction path", () => {
		it("stamps `tree_sitter` on contracts produced via AST walking", async () => {
			...
			expect(consumers).toHaveLength(1);
			expect((consumers[0].meta as { source?: string } | undefined)?.source).toBe(
				"tree_sitter",
			);
		});
	...
	describe("normalizeContractId for include", () => {
		it("lowercases the path", () => {
			expect(normalizeContractId("include::Map/Base/Foo.h")).toBe(
				"include::map/base/foo.h",
			);
		});
...
int main(){return 0;}`,
	...
	describe("follow-up: extractProvidersGraph strips repo root from absolute paths", () => {
		...
		it("drops graph rows whose path resolves outside the repo root", async () => {
			...
			expect(providers.map((p) => p.contractId)).toEqual([
