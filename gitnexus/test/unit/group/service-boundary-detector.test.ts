...
describe("ServiceBoundaryDetector", () => {
	...
	describe("assignService", () => {
		it("test_assign_file_to_correct_service", () => {
			...
			expect(assignService("services/auth/src/index.ts", boundaries)).toBe(
				"services/auth",
			);
			expect(assignService("services/orders/src/main.ts", boundaries)).toBe(
				"services/orders",
			);
		});
		...
		it("test_assign_nested_file_uses_deepest_boundary", () => {
			...
			expect(
				assignService("platform/services/auth/src/index.ts", boundaries),
			).toBe("platform/services/auth");
			expect(assignService("platform/shared/utils.ts", boundaries)).toBe(
				"platform",
			);
		});
