...
import { stableRepoPoolId, syncGroup } from "../../../src/core/group/sync.js";
import type {
	GroupConfig,
	GroupManifestLink,
	RepoHandle,
...
describe("syncGroup", () => {
	...
	it("test_syncGroup_closes_only_opened_pools", async () => {
		...
		try {
			...
			const blanketCalls = closeSpy.mock.calls.filter(
	...
	it("manifest links in config.links produce cross-links with matchType manifest", async () => {
		...
		expect(result.contracts).toHaveLength(2);
		const manifestLinks = result.crossLinks.filter(
			(cl) => cl.matchType === "manifest",
		);
		...
		expect(manifestLinks[0].from.symbolUid).toBe(
			"manifest::app/consumer::http::GET::/api/orders",
		);
		expect(manifestLinks[0].to.symbolUid).toBe(
			"manifest::app/provider::http::GET::/api/orders",
		);
		...
		const exactForSameContract = result.crossLinks.filter(
			(cl) =>
				cl.matchType === "exact" && cl.contractId === "http::GET::/api/orders",
		);
	...
	it("keeps wildcard thrift links to multiple extracted IDL provider methods", async () => {
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-sync-thrift-wildcard-"),
		);
  ...
  OrderResponse GetOrder(1: string orderId)
	...
	it("matches weak thrift method consumers to namespace-qualified providers during sync", async () => {
		...
		const consumer: StoredContract = {
			...
			symbolRef: {
				filePath: "src/BillingWorkflow.java",
		...
		expect(result.crossLinks).toHaveLength(1);
		expect(result.crossLinks[0].matchType).toBe("exact");
		expect(result.crossLinks[0].contractId).toBe(
	...
	it("keeps exact thrift links to extracted IDL and Java providers for same method", async () => {
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-sync-thrift-exact-"),
		);
		fs.mkdirSync(path.join(tmpDir, "idl"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "src", "main", "java", "example"), {
			recursive: true,
		});
		fs.writeFileSync(
			path.join(tmpDir, "idl", "order.thrift"),
  ...
  public PlaceOrderResponse PlaceOrder(PlaceOrderRequest request) {
    return new PlaceOrderResponse();
		...
		try {
			...
			expect(result.crossLinks).toHaveLength(2);
			expect(
				result.crossLinks.map((cl) => cl.to.symbolRef.filePath).sort(),
			).toEqual([
				"idl/order.thrift",
	...
	it("extracts thrift contracts during real sync when thrift detection is enabled", async () => {
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-sync-thrift-"),
		);
		const storageDir = path.join(tmpDir, ".gitnexus");
		fs.mkdirSync(path.join(tmpDir, "services", "billing", "idl"), {
			recursive: true,
		});
		fs.mkdirSync(path.join(tmpDir, "services", "billing", "src"), {
			recursive: true,
		});
		fs.mkdirSync(storageDir, { recursive: true });
		fs.writeFileSync(
			path.join(tmpDir, "services", "billing", "package.json"),
			"{}",
		);
		fs.writeFileSync(
...
service OrderService {
  PlaceOrderResponse PlaceOrder(1: PlaceOrderRequest request)
	...
	it("does not extract thrift contracts during real sync when thrift detection is disabled", async () => {
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-sync-thrift-off-"),
		);
		const storageDir = path.join(tmpDir, ".gitnexus");
		fs.mkdirSync(path.join(tmpDir, "services", "billing", "idl"), {
			recursive: true,
...
service OrderService {
  PlaceOrderResponse PlaceOrder(1: PlaceOrderRequest request)
	...
	it("does not extract include contracts during real sync when includes detection is disabled", async () => {
		...
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gitnexus-sync-includes-off-"),
		);
		const storageDir = path.join(tmpDir, ".gitnexus");
		fs.mkdirSync(path.join(tmpDir, "src"), { recursive: true });
		fs.mkdirSync(storageDir, { recursive: true });
		fs.writeFileSync(
			path.join(tmpDir, "src", "view.h"),
			"#pragma once\nclass View {};",
		);
	...
	it("manifest links referencing unknown repos still produce cross-links via synthetic UIDs", async () => {
		...
		try {
			...
			expect(
				cap.records().some((r) => String(r.msg ?? "").includes("app/dangling")),
			).toBe(true);
		} finally {
			cap.restore();
		}
