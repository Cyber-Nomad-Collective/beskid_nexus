...
describe("ManifestExtractor", () => {
			...
			[
				...
				async (_cypher, params) => {
					if (
						...
						params?.serviceName === "auth.AuthService"
					) {
						return [
							{
	...
	it("resolves lib manifest links by exact name only", async () => {
		...
		expect(consumer?.symbolUid).toBe(
			"manifest::platform/web::lib::@platform/contracts",
		);
	});
	...
	it("returns null cleanly when no Route matches explicit-method http contract", async () => {
		...
		expect(provider?.symbolUid).toBe(
			"manifest::orders-svc::http::GET::/api/orders",
		);
	});
	...
	it("canonicalizes method casing so get::/api/orders and GET::/api/orders share a contractId", async () => {
		...
		const lowerContractId = lower.contracts.find(
			(c) => c.role === "provider",
		)?.contractId;
		const upperContractId = upper.contracts.find(
			(c) => c.role === "provider",
		)?.contractId;
		...
		>([
			...
			[
				...
				async (_cypher, params) => {
					if (params?.symbolName === "Token") {
						return [
							{ uid: "uid-token-consumer", name: "Token", filePath: "src/lexer.rs" },
						];
					}
					return [];
				},
	...
	it("memoizes repeated (repo, type, contract) resolutions so each tuple hits the DB once", async () => {
		...
		const dbExecutors = new Map<
			string,
			(c: string) => Promise<Record<string, unknown>[]>
		>([
			["svc/a", execFor("svc/a")],
			["svc/b", execFor("svc/b")],
		]);
		...
		expect(new Set(calls.map((c) => c.repo))).toEqual(
			new Set(["svc/a", "svc/b"]),
		);
	});
});
