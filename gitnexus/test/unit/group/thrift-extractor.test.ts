...
service OrderService {
  PlaceOrderResponse PlaceOrder(1: PlaceOrderRequest request)
		...
		expect(contracts[0].contractId).toBe(
			"thrift::billing.v1.OrderService/PlaceOrder",
		);
...
service OrderService {
  PlaceOrderResponse PlaceOrder(1: PlaceOrderRequest request)
		...
		expect(contracts[0].contractId).toBe(
			"thrift::billing_python.v1.OrderService/PlaceOrder",
		);
...
`,
		...
		await expect(
			extractor.extract(null, tmpDir, makeRepo(tmpDir)),
		).resolves.toEqual([]);
	});
  ...
  void submit(PlaceOrderRequest request) throws Exception {
    ...
    generatedOrderService.PlaceOrder(request);
		...
		for (const contract of consumers) {
			...
			expect(contract.symbolRef.filePath).toBe(
				"src/main/java/example/BillingWorkflow.java",
			);
		}
	});
...
class IfaceOrderHandler implements OrderService.Iface {
  public PlaceOrderResponse PlaceOrder(PlaceOrderRequest request) {
    return new PlaceOrderResponse();
		...
		expect(providers).toHaveLength(2);
		expect(providers.map((c) => c.symbolUid)).toEqual(
			repeatedProviders.map((c) => c.symbolUid),
		);
		expect(providers.every((c) => c.symbolUid.length > 0)).toBe(true);
		expect(new Set(providers.map((c) => c.symbolUid)).size).toBe(2);
		expect(
			providers.every((c) => !c.symbolUid.includes("::thrift::billing.v1")),
		).toBe(true);
	});
  ...
  public PlaceOrderResponse PlaceOrder(PlaceOrderRequest request) {
    return new PlaceOrderResponse();
		...
		const providers = contracts.filter(
			(c) => c.meta.source === "java_thrift_provider",
		);
...
class BillingWorkflow {
  ...
  void submit(PlaceOrderRequest request) throws Exception {
    orderService.PlaceOrder(request);
		...
		expect(contracts[0].symbolRef.filePath).toBe(
			"src/main/java/example/BillingWorkflow.java",
		);
	});
...
describe("buildThriftContext", () => {
	...
	beforeEach(async () => {
		tmpDir = await fsp.mkdtemp(
			path.join(os.tmpdir(), "gitnexus-thrift-context-"),
		);
	});
...
describe("Thrift contract id helpers", () => {
	it("test_thriftMethodContractId_with_namespace", () => {
		expect(
			thriftMethodContractId("billing.v1", "OrderService", "PlaceOrder"),
		).toBe("thrift::billing.v1.OrderService/PlaceOrder");
	});
	...
	it("test_thriftServiceContractId_without_namespace", () => {
		expect(thriftServiceContractId("", "OrderService")).toBe(
			"thrift::OrderService/*",
		);
	});
});
