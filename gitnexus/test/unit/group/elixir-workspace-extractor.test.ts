...
describe("ElixirWorkspaceExtractor", () => {
	...
	it("handles grouped alias (alias MyApp.{ModA, ModB})", async () => {
		...
		await writeFile(
			"shared/lib/shared/config.ex",
			"defmodule Shared.Config do\nend\n",
		);
		await writeFile(
			"shared/lib/shared/logger.ex",
	...
	it("collapses nested submodules to top-level module contract", async () => {
		...
		await writeFile(
			"core/lib/core/auth/token.ex",
			"defmodule Core.Auth.Token do\nend\n",
		);
		await writeFile(
			"core/lib/core/auth/session.ex",
			"defmodule Core.Auth.Session do\nend\n",
		);
