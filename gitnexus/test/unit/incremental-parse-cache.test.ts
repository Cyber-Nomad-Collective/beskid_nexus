...
const minimalResult = (
	overrides: Partial<ParseWorkerResult> = {},
): ParseWorkerResult => ({
	nodes: [],
	relationships: [],
	symbols: [],
...
describe("loadParseCache / saveParseCache (round-trip)", () => {
	it("round-trips an empty cache", async () => {
		...
		try {
			...
			await expect(
				fs.access(path.join(dir, "parse-cache", "index.json")),
			).resolves.toBeUndefined();
			await expect(
				fs.access(path.join(dir, "parse-cache.json")),
			).rejects.toThrow();
	...
	it("skips corrupt or missing shards while loading the sharded cache", async () => {
		...
		try {
			...
			await fs.writeFile(
				path.join(cacheDir, `${badKey}.json`),
				"{not-json",
				"utf-8",
			);
	...
	it("round-trips Map and Set values through the JSON replacer/reviver", async () => {
		...
		try {
			...
			const scope = (reloaded as ParseWorkerResult).parsedFiles[0]
				?.scopes[0] as unknown as {
	...
	it("writes one shard file per cache entry (three distinct keys)", async () => {
		...
		try {
			...
			expect(
				names.filter((n) => n.endsWith(".json") && n !== "index.json").length,
			).toBe(3);
			const loaded = await loadParseCache(dir);
