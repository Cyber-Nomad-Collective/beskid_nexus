...
describe("contentHashForNode", () => {
	const makeNode = (
		overrides: Partial<EmbeddableNode> = {},
	): EmbeddableNode => ({
	...
	it("produces identical hash regardless of config vs finalConfig when config is empty", () => {
		...
		const hashWithFullDefaults = contentHashForNode(
			node,
			DEFAULT_EMBEDDING_CONFIG,
		);
...
describe("EMBEDDING_INDEX_NAME", () => {
	it("is exported from schema.ts", async () => {
		const { EMBEDDING_INDEX_NAME } = await import(
...
describe("runEmbeddingPipeline incremental filter", () => {
	...
	const mockEmbedderSetup = () => {
		vi.doMock("../../src/core/embeddings/embedder.js", () => ({
			...
			embeddingToArray: vi
				.fn()
				.mockImplementation((emb: Float32Array) => Array.from(emb)),
	...
	const mockExecuteWithReusedStatement = () => {
		return vi
			.fn()
			.mockImplementation(
				async (cypher: string, params: Array<Record<string, any>>) => {
	...
	it("maps positional query rows with description/isExported columns correctly", async () => {
		...
		const embeddedTexts = embedBatchSpy.mock.calls.flatMap(
			(call) => call[0] as string[],
		);
		const classText = embeddedTexts.find((text) =>
			text.includes("Class: Parser"),
		);
	...
	it("deletes and re-embeds stale nodes (hash mismatch)", async () => {
		...
		expect(deleteCalls[0].params.some((p: any) => p.nodeId === node.id)).toBe(
			true,
		);
	...
	it("calls createVectorIndex even when zero nodes need embedding after filter", async () => {
		...
		const vectorIndexCalls = queryCalls.filter((c) =>
			c.includes("CREATE_VECTOR_INDEX"),
		);
  ...
  validate() { return true; }
		...
		const laterChunk = embeddedTexts.find((text) =>
			text.includes("[preceding context]:"),
		);
	...
	it("throws when DELETE for stale nodes fails with non-trivial error", async () => {
		...
		const executeQuery = mockExecuteQuery([node]);
		const executeWithReusedStatement = vi
			.fn()
