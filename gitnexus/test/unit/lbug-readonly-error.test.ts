...
describe("isReadOnlyDbError", () => {
	...
	it('is case-insensitive on the "read-only" substring', () => {
		expect(isReadOnlyDbError(new Error("Read-Only Database access denied"))).toBe(
			true,
		);
	});
	...
	it("accepts non-Error values (string, unknown) without throwing", () => {
		...
		expect(isReadOnlyDbError({ toString: () => "read-only database" })).toBe(
			true,
		);
	...
	it("does NOT match unrelated errors that the ensure path must still surface", () => {
		...
		expect(isReadOnlyDbError(new Error("Could not set lock on file"))).toBe(
			false,
		);
		// "already exists" — the happy idempotent path inside createFTSIndex.
		expect(isReadOnlyDbError(new Error("Index file_fts already exists"))).toBe(
			false,
		);
