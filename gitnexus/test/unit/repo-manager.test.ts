...
describe("ensureGitNexusIgnored (#1233)", () => {
	...
	it("does not create or modify the repository root .gitignore", async () => {
		...
		await expect(fs.readFile(rootGitignorePath, "utf-8")).resolves.toBe(
			"node_modules/\n",
		);
	});
	...
	it("does not create .git/info/exclude when .git is not a directory", async () => {
		...
		await expect(
			fs.access(path.join(tmpRepo.dbPath, ".git", "info", "exclude")),
		).rejects.toThrow();
	});
	...
	it("keeps generated .gitnexus files out of git status", async () => {
		...
		await ensureGitNexusIgnored(tmpRepo.dbPath);
		await fs.writeFile(
			path.join(tmpRepo.dbPath, ".gitnexus", "meta.json"),
			"{}\n",
		);
...
describe("registerRepo name override + collision guard (#829)", () => {
	...
	it("registerRepo throws RegistryNameCollisionError when another path uses the name", async () => {
		...
		await expect(
			registerRepo(tmpRepoB.dbPath, meta, { name: "shared" }),
	...
	it("registerRepo({ name, allowDuplicateName: true }) allows the duplicate to coexist", async () => {
		await registerRepo(tmpRepoA.dbPath, meta, { name: "shared" });
		await registerRepo(tmpRepoB.dbPath, meta, {
			name: "shared",
		...
		expect(paths).toEqual(
			[path.resolve(tmpRepoA.dbPath), path.resolve(tmpRepoB.dbPath)].sort(),
		);
	});
...
describe("parseRepoNameFromUrl", () => {
	it("parses HTTPS URLs and strips .git", () => {
		expect(parseRepoNameFromUrl("https://github.com/owner/lume_spark.git")).toBe(
			"lume_spark",
		);
		expect(parseRepoNameFromUrl("https://github.com/owner/lume_spark")).toBe(
			"lume_spark",
		);
	});

	it("parses SSH URLs (git@host:owner/repo.git)", () => {
		expect(parseRepoNameFromUrl("git@github.com:owner/lume_spark.git")).toBe(
			"lume_spark",
		);
		expect(parseRepoNameFromUrl("git@gitlab.com:group/sub/lume_spark.git")).toBe(
			"lume_spark",
		);
	});
	...
	it("parses ssh:// and git:// URLs", () => {
		expect(
			parseRepoNameFromUrl("ssh://git@host.example/owner/lume_spark.git"),
		).toBe("lume_spark");
		expect(parseRepoNameFromUrl("git://host.example/owner/lume_spark.git")).toBe(
			"lume_spark",
		);
	});

	it("parses local file:// URLs", () => {
		expect(parseRepoNameFromUrl("file:///srv/git/lume_spark.git")).toBe(
			"lume_spark",
		);
	});
...
describe("resolveRegistryEntry (#664)", () => {
	...
	it("path match is case-insensitive on Windows only", () => {
		if (process.platform !== "win32") {
			...
			expect(() => resolveRegistryEntry(entries, upper)).toThrow(
				RegistryNotFoundError,
			);
			return;
		}
	...
	it("throws RegistryAmbiguousTargetError when name matches multiple entries", () => {
		...
		expect(() => resolveRegistryEntry(entries, "app")).toThrow(
			RegistryAmbiguousTargetError,
		);
		try {
	...
	it("throws RegistryNotFoundError when no entry matches", () => {
		expect(() => resolveRegistryEntry(entries, "nonexistent")).toThrow(
			RegistryNotFoundError,
		);
		try {
...
describe("assertSafeStoragePath (#1003)", () => {
	...
	it("UnsafeStoragePathError carries the original entry + expected + actual paths", () => {
		...
		try {
			...
			expect(err.expectedStoragePath).toBe(
