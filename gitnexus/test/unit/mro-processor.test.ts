...
describe("computeMRO", () => {
	...
	describe("METHOD_IMPLEMENTS edges", () => {
		...
		it("matches overloaded interface methods by parameterTypes", () => {
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Interface", "IRepo")}->${ifaceFind1}`,
				),
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Interface", "IRepo")}->${ifaceFind2}`,
				),
				sourceId: generateId("Interface", "IRepo"),
				targetId: ifaceFind2,
				type: "HAS_METHOD",
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Class", "SqlRepo")}->${sqlFind1Id}`,
				),
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Class", "SqlRepo")}->${sqlFind2Id}`,
				),
				sourceId: generateId("Class", "SqlRepo"),
				targetId: sqlFind2Id,
				type: "HAS_METHOD",
		...
		it("skips Property nodes on interface", () => {
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Interface", "IFoo")}->${propId}`,
				),
		...
		describe("METHOD_IMPLEMENTS inherited + arity matching", () => {
			...
			it("arity mismatch prevents false match", () => {
				...
				graph.addRelationship({
					id: generateId(
						"HAS_METHOD",
						`${generateId("Interface", "IArity")}->${iMethodId}`,
					),
				...
				graph.addRelationship({
					id: generateId(
						"HAS_METHOD",
						`${generateId("Class", "CArity")}->${cMethodId}`,
					),
			...
			it("arity match when types missing", () => {
				...
				graph.addRelationship({
					id: generateId(
						"HAS_METHOD",
						`${generateId("Interface", "IArityOk")}->${iMethodId}`,
					),
				...
				graph.addRelationship({
					id: generateId(
						"HAS_METHOD",
						`${generateId("Class", "CArityOk")}->${cMethodId}`,
					),
			...
			it("multiple same-arity candidates = ambiguous, no edge emitted", () => {
				...
				graph.addRelationship({
					id: generateId(
						"HAS_METHOD",
						`${generateId("Interface", "IAmbig")}->${iMethodId}`,
					),
				...
				graph.addRelationship({
					id: generateId(
						"HAS_METHOD",
						`${generateId("Class", "CAmbig")}->${cMethod1}`,
					),
				...
				graph.addRelationship({
					id: generateId(
						"HAS_METHOD",
						`${generateId("Class", "CAmbig")}->${cMethod2}`,
					),
					sourceId: generateId("Class", "CAmbig"),
					targetId: cMethod2,
					type: "HAS_METHOD",
	...
	describe("findInheritedMethod ambiguity", () => {
		it("returns null when two EXTENDS parents both provide matching method", () => {
			...
			const fooEdges = mi.filter(
				(e) => graph.getNode(e.targetId)?.properties.name === "foo",
			);
	...
	describe("default interface method resolution", () => {
		it("interface default method satisfies grandparent interface contract", () => {
			...
			const i1Bar = addMethod(graph, "I1Def", "bar", "Interface", undefined, {
				isAbstract: true,
			});
		...
		it("own method takes priority over interface default", () => {
			...
			const iBar = addMethod(graph, "IOwn", "bar", "Interface", undefined, {
				isAbstract: false,
			});
		...
		it("transitive interface default: C implements I2, I2 extends I1, I1 has abstract bar, I2 has default bar → I2.bar satisfies I1.bar", () => {
			...
			// I1 has abstract bar
			const i1Bar = addMethod(graph, "I1", "bar", "Interface", undefined, {
				isAbstract: true,
			});
	...
	describe("METHOD_IMPLEMENTS confidence tiering", () => {
		it("fully-typed match gets confidence 1.0", () => {
			...
			const iFoo = addMethod(graph, "ITyped", "foo", "Interface", [
				"int",
				"String",
			]);
		...
		it("arity-only match (both have parameterCount, no types) gets confidence 1.0", () => {
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Interface", "IArity")}->${iBarId}`,
				),
				sourceId: generateId("Interface", "IArity"),
				targetId: iBarId,
				type: "HAS_METHOD",
		...
		it("lenient match (no types, no parameterCount on both sides) gets confidence 0.7", () => {
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Interface", "ILenient")}->${iBazId}`,
				),
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Class", "CLenient")}->${cBazId}`,
				),
		...
		it("one side has parameterCount, other does not → confidence 0.7", () => {
			...
			graph.addRelationship({
				id: generateId(
					"HAS_METHOD",
					`${generateId("Interface", "IHalf")}->${iQuxId}`,
				),
	...
	describe("IMPLEMENTS BFS ambiguity for default methods", () => {
		it("does not emit METHOD_IMPLEMENTS when two interfaces provide same default method (ambiguous)", () => {
			...
			addMethod(graph, "IAncestor", "process", "Interface", undefined, {
				isAbstract: true,
			});
		...
		it("emits METHOD_IMPLEMENTS when only one interface provides the default method (unambiguous)", () => {
			...
			const ancestorProcess = addMethod(
				graph,
				"IAncestor",
				"process",
				"Interface",
