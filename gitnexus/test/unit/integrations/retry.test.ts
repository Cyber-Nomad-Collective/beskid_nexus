...
describe("computeBackoffMs", () => {
	...
	it("full-jitter delay falls within [0, min(cap, base * 2^attempt)]", () => {
		...
		expect(computeBackoffMs(0, 500, 5000, undefined, () => 0.999)).toBeLessThan(
			500,
		);
		...
		expect(computeBackoffMs(4, 500, 5000, undefined, () => 0.999)).toBeLessThan(
			5000,
		);
	});
});
...
describe("withRetry", () => {
	...
	it("throws the last error when maxAttempts exhausted", async () => {
		...
		await expect(
			withRetry(fn, makeOpts({ sleep, maxAttempts: 3 })),
	...
	it("rejects maxAttempts < 1", async () => {
		await expect(
			withRetry(async () => "ok", makeOpts({ maxAttempts: 0 })),
