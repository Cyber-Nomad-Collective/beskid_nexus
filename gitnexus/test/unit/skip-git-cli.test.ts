...
describe("--skip-git CLI flag", () => {
	...
	it("warns when --index-only overrides --skills (PR 1485)", () => {
		...
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "gn-index-only-skills-"),
		);
		const gitnexusHome = fs.mkdtempSync(
			path.join(os.tmpdir(), "gn-index-only-skills-home-"),
		);
	...
	it("still respects .gitnexusignore when run with --skip-git", () => {
		...
		const gitnexusHome = fs.mkdtempSync(
			path.join(os.tmpdir(), "gn-skip-git-ignore-home-"),
		);
		...
		try {
			execSync(
				`node "${cliPath}" analyze "${tmpDir}" --skip-git --skip-agents-md`,
				{
					encoding: "utf8",
					timeout: 60000,
					env,
	...
	describe("--skip-git does not walk up to parent git repo (#1232)", () => {
		...
		function expectCoolioRegistryEntry() {
			...
			expect(canonicalPath(entry.path)).toBe(
				canonicalPath(path.join(parentDir, "COOLIO")),
			);
			expect(
			...
			expect(
				registry.find(
					(e) =>
						...
						canonicalPath(path.join(parentDir, "SubWooder")),
				),
			).toBeUndefined();
			return entry;
		}
		...
		it("from subdir inside parent git repo, indexes subdir not parent", () => {
			...
			try {
				...
				const siblingQuery = execSync(
					`node "${cliPath}" query bass --repo COOLIO`,
					{
						cwd: path.join(parentDir, "COOLIO"),
						encoding: "utf8",
						timeout: 60000,
		...
		it("keeps parent git status clean for --skip-git subdir analyze (#1233)", () => {
			...
			try {
				...
				execSync("git add .gitignore COOLIO SubWooder", {
					cwd: parentDir,
					stdio: "ignore",
				});
				execSync(
				...
				expect(
					fs.readFileSync(
						path.join(parentDir, "COOLIO", ".gitnexus", ".gitignore"),
						"utf8",
					),
				).toBe("*\n");
