...
vi.mock("../../src/core/lbug/pool-adapter.js", () => ({
	executeQuery: (repoId: string, cypher: string) =>
		mockExecuteQuery(repoId, cypher),
...
describe("BM25 search", () => {
	describe("createSearchFTSIndexes", () => {
		...
		it("creates the configured indexes on the writable analysis path", async () => {
			const { createFTSIndex } = await import(
				"../../src/core/lbug/lbug-adapter.js"
			);
			const { createSearchFTSIndexes } = await import(
	...
	describe("MCP pool path", () => {
		...
		it("queries existing FTS indexes without issuing CREATE_FTS_INDEX", async () => {
			mockExecuteQuery.mockImplementation(
				async (_repo: string, cypher: string) => {
					...
					if (cypher.includes("QUERY_FTS_INDEX('Function'")) {
						return [
							{ node: { filePath: "src/auth.ts", id: "func:login" }, score: 8 },
						];
					}
					return [];
				},
			...
			expect(
				mockExecuteQuery.mock.calls.some((c) =>
		...
		it("uses the configured FTS query set on every call", async () => {
			...
			expect(
				queryCalls.map(
					(c) => String(c[1]).match(/QUERY_FTS_INDEX\('([^']+)'/)?.[1],
				),
			).toEqual(["File", "Function", "Class", "Method", "Interface"]);
		});
	});
});
