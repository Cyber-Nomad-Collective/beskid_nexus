...
describe("C++ MethodExtractor", () => {
	...
	describe("extract", () => {
      ...
      `);
			const structResult = extractor.extract(
  ...
  class << self
    def bar; end
      ...
      `);
			...
			const bodyStmt = classNode.namedChildren.find(
				(c) => c.type === "body_statement",
			)!;
			const singletonClass = bodyStmt.namedChildren.find(
				(c) => c.type === "singleton_class",
			)!;
...
describe("PHP MethodExtractor", () => {
	...
	describe("isTypeDeclaration", () => {
		it("recognizes class_declaration", () => {
			...
			const cls = tree.rootNode.namedChildren.find(
		...
		it("recognizes interface_declaration", () => {
			...
			const iface = tree.rootNode.namedChildren.find(
				(c) => c.type === "interface_declaration",
			)!;
		...
		it("recognizes trait_declaration", () => {
			...
			const trait = tree.rootNode.namedChildren.find(
				(c) => c.type === "trait_declaration",
			)!;
...
enum Status: string {
    ...
    public function label(): string {
      ...
      `);
			const enumNode = tree.rootNode.namedChildren.find(
				(c) => c.type === "enum_declaration",
			)!;
...
describe("Rust MethodExtractor", () => {
	...
	describe("extract from impl", () => {
		...
		it("marks associated function (no self) as static", () => {
			const code = [
				"impl Config {",
				"  pub fn new(path: String) -> Config { todo!() }",
				"}",
			].join("\n");
			const tree = parseRust(code);
			const implNode = tree.rootNode.child(0)!;
	...
	describe("impl Trait for Struct owner resolution", () => {
		it("attributes methods to the concrete Struct, not the Trait", () => {
			...
			const implNode = tree.rootNode.namedChildren.find(
				(c) => c.type === "impl_item",
			)!;
			const result = extractor.extract(implNode, rustCtx);
...
type Animal interface {
    Speak() string
      ...
      `);
			const typeDecl = tree.rootNode.namedChildren.find(
				(c) => c.type === "type_declaration",
			)!;
...
const extractor_kotlin = Kotlin
	? createMethodExtractor(kotlinMethodConfig)
	: null;
...
function makeStubConfig(
	overrides: Partial<MethodExtractionConfig> = {},
