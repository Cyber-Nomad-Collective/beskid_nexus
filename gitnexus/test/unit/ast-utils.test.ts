...
const { createParserForLanguage, getLanguageFromFilename, parseSourceSafeSpy } =
	vi.hoisted(() => ({
...
describe("ensureAndParse", () => {
	...
	it("reuses the parser for the same grammar key across interleaved languages", async () => {
		...
		createParserForLanguage.mockImplementation(
			async (language: string, filePath?: string) => {
		...
		const { ensureAndParse } = await import(
			"../../src/core/embeddings/ast-utils.js"
		);
	...
	it("uses separate parser instances for .ts and .tsx", async () => {
		...
		createParserForLanguage.mockImplementation(
			async (_language: string, filePath?: string) => {
