...
import {
	resolveImportPath,
	stripJsExtension,
} from "../../src/core/ingestion/import-resolvers/standard.js";
...
describe("stripJsExtension", () => {
	...
	it("strips .jsx", () =>
		expect(stripJsExtension("foo/bar.jsx")).toBe("foo/bar"));
	it("strips .mjs", () =>
		expect(stripJsExtension("foo/bar.mjs")).toBe("foo/bar"));
	it("strips .cjs", () =>
		expect(stripJsExtension("foo/bar.cjs")).toBe("foo/bar"));
	it("returns null for .ts", () =>
		expect(stripJsExtension("foo/bar.ts")).toBeNull());
	it("returns null for no extension", () =>
		expect(stripJsExtension("foo/bar")).toBeNull());
});
