...
describe("git utilities", () => {
	...
	describe("findGitRootByDotGit", () => {
		...
		it("returns null when the input path does not exist", () => {
			const missing = path.join(
				os.tmpdir(),
				`gitnexus-missing-${Date.now()}-${Math.random()}`,
			);
	...
	describe("parseRepoNameFromUrl", () => {
		it("extracts and sanitizes name from HTTPS URL", () => {
			expect(parseRepoNameFromUrl("https://github.com/user/my-repo.git")).toBe(
				"my-repo",
			);
			expect(parseRepoNameFromUrl("https://github.com/user/--payload.git")).toBe(
				"payload",
			);
		});

		it("extracts and sanitizes name from SSH URL", () => {
			expect(parseRepoNameFromUrl("git@github.com:user/my-repo.git")).toBe(
				"my-repo",
			);
