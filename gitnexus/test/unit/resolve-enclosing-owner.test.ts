...
class Animal
  class << self
    ...
    `);
		...
		const innerBody = singletonClass.namedChildren.find(
			(c) => c.type === "body_statement",
		)!;
...
module Helpers
  class << self
    ...
    `);
		...
		const bodyStmt = moduleNode.namedChildren.find(
			(c) => c.type === "body_statement",
		)!;
		const singletonClass = bodyStmt.namedChildren.find(
			(c) => c.type === "singleton_class",
...
describe("Kotlin enclosing owner resolution (no resolveEnclosingOwner needed)", () => {
	(Kotlin ? it : it.skip)(
      ...
      `);
			...
			const companionBody = companion.namedChildren.find(
				(c) => c.type === "class_body",
			)!;
	...
	(Kotlin ? it : it.skip)(
      ...
      `);
			...
			const funcDecl = objBody.namedChildren.find(
				(c) => c.type === "function_declaration",
			)!;
...
class Outer
  class Inner
    ...
    `);
		...
		const outerBody = outerClass.namedChildren.find(
			(c) => c.type === "body_statement",
		)!;
		const innerClass = outerBody.namedChildren.find((c) => c.type === "class")!;
		const innerBody = innerClass.namedChildren.find(
			(c) => c.type === "body_statement",
		)!;
		const methodNode = innerBody.namedChildren.find((c) => c.type === "method")!;
		...
		const info = findEnclosingClassInfo(methodNode, "nested.rb", (node) => {
			...
			if (calls > 50)
				throw new Error("hook called too many times — possible infinite loop");
		...
		expect(info === null || (info && typeof info.className === "string")).toBe(
			true,
		);
	});
