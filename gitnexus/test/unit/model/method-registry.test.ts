...
describe("MethodRegistry — basic lookup", () => {
	...
	it("isolates methods by ownerNodeId — same method name on two classes does not collide", () => {
		...
		expect(reg.lookupMethodByOwner("class:User", "save")?.nodeId).toBe(
			"method:User.save",
		);
		expect(reg.lookupMethodByOwner("class:Order", "save")?.nodeId).toBe(
			"method:Order.save",
		);
	});
});
...
describe("MethodRegistry — arity narrowing", () => {
	it("narrows overloads by parameterCount when argCount is provided", () => {
		const reg = createMethodRegistry();
		const greetEmpty = makeMethod({
			nodeId: "method:greet#0",
			parameterCount: 0,
		});
		...
		expect(reg.lookupMethodByOwner("class:User", "greet", 0)?.nodeId).toBe(
			"method:greet#0",
		);
		// argCount 1 matches only the 1-arg overload
		expect(reg.lookupMethodByOwner("class:User", "greet", 1)?.nodeId).toBe(
			"method:greet#1",
		);
	});
	...
	it("arity fallback — when no overload matches argCount, returns from the full pool (testing reviewer T-01)", () => {
		...
		const reg = createMethodRegistry();
		const save1 = makeMethod({
			nodeId: "method:save#1",
			parameterCount: 1,
			returnType: "void",
		});
		const save2 = makeMethod({
			nodeId: "method:save#2",
			parameterCount: 2,
			returnType: "void",
		});
	...
	it("requiredParameterCount range — argCount between required and total is accepted (testing reviewer T-02)", () => {
		...
		expect(reg.lookupMethodByOwner("class:Calc", "bar", 1)?.nodeId).toBe(
			"method:bar",
		);
		expect(reg.lookupMethodByOwner("class:Calc", "bar", 2)?.nodeId).toBe(
			"method:bar",
		);
		expect(reg.lookupMethodByOwner("class:Calc", "bar", 3)?.nodeId).toBe(
			"method:bar",
		);
		// argCount 5 matches the second overload only
		expect(reg.lookupMethodByOwner("class:Calc", "bar", 5)?.nodeId).toBe(
			"method:bar#other",
		);
	});
...
describe("MethodRegistry — return-type dedup", () => {
	it("returns first when all overloads share the same returnType", () => {
		...
		expect(reg.lookupMethodByOwner("class:X", "compute", 1)?.nodeId).toBe(
			"method:a",
		);
	});
	...
	it("single-overload methods skip the dedup path", () => {
		...
		expect(reg.lookupMethodByOwner("class:X", "only", 1)?.nodeId).toBe(
			"method:only",
		);
	});
});
...
describe("MethodRegistry — lookupMethodByName", () => {
	...
	it("accumulates overloads under the same owner", () => {
		const reg = createMethodRegistry();
		const overload1 = makeMethod({
			nodeId: "method:User.save#0",
			parameterCount: 0,
		});
		const overload2 = makeMethod({
			nodeId: "method:User.save#1",
			parameterCount: 1,
		});
	...
	it("does not return methods with different names", () => {
		...
		reg.register(
			"class:User",
			"save",
			makeMethod({ nodeId: "method:User.save" }),
		);
		reg.register(
			"class:User",
			"load",
			makeMethod({ nodeId: "method:User.load" }),
		);
...
describe("hasFunctionMethods flag", () => {
	...
	it("stays false after registering only strict-Method defs", () => {
		...
		reg.register(
			"class:User",
			"save",
			makeMethod({ nodeId: "method:User.save", type: "Method" }),
		);
		reg.register(
			"class:User",
			"load",
	...
	it("flips to true when a Function-typed def (Python/Rust/Kotlin class method) is registered", () => {
		...
		reg.register(
			"class:User",
			"greet",
			makeMethod({ nodeId: "fn:User.greet", type: "Function" }),
		);
	...
	it("stays true after further strict-Method registrations", () => {
		...
		reg.register(
			"class:Dog",
			"bark",
			makeMethod({ nodeId: "method:Dog.bark", type: "Method" }),
		);
		expect(reg.hasFunctionMethods).toBe(true);
	});
