...
describe("isAzureProvider", () => {
	it("returns true for .openai.azure.com URLs", () => {
		expect(isAzureProvider("https://myresource.openai.azure.com/openai/v1")).toBe(
			true,
		);
	});
	...
	it("returns true for .services.ai.azure.com URLs", () => {
		expect(
			isAzureProvider("https://myresource.services.ai.azure.com/openai/v1"),
		).toBe(true);
	});
	...
	it("returns false for spoofed URLs containing azure hostname as subdomain", () => {
		expect(
			isAzureProvider("https://myresource.openai.azure.com.evil.com/v1"),
		).toBe(false);
	});
});
...
describe("buildRequestUrl", () => {
	...
	it("appends api-version query param when provided", () => {
		expect(
			buildRequestUrl(
				"https://myres.openai.azure.com/openai/deployments/dep1",
				"2024-10-21",
			),
		).toBe(
	...
	it("does not append api-version when undefined", () => {
		expect(
			buildRequestUrl("https://myres.openai.azure.com/openai/v1", undefined),
		).toBe("https://myres.openai.azure.com/openai/v1/chat/completions");
	});
});
...
describe("callLLM — Azure content_filter error", () => {
	...
	it("throws a clear error when Azure returns content_filter 400", async () => {
		const fetchSpy = vi.fn().mockResolvedValue(
			new Response(
				JSON.stringify({
					error: { code: "content_filter", message: "Prompt triggered policy" },
				}),
...
describe("validateLLMBaseUrl", () => {
	it("allows https:// for any public host", () => {
		...
		expect(() =>
			validateLLMBaseUrl("https://openrouter.ai/api/v1"),
		).not.toThrow();
		expect(() =>
			validateLLMBaseUrl("https://myres.openai.azure.com/openai/v1"),
		).not.toThrow();
	});
	...
	it("rejects http:// for non-loopback hosts", () => {
		...
		expect(() => validateLLMBaseUrl("http://192.168.1.1/v1")).toThrow(
			"Insecure http://",
		);
		// Private IP ranges
		expect(() => validateLLMBaseUrl("http://10.0.0.1/v1")).toThrow(
			"Insecure http://",
		);
		// AWS/GCP IMDS — should be blocked
		expect(() =>
			validateLLMBaseUrl("http://169.254.169.254/latest/meta-data"),
		).toThrow("Insecure http://");
	});

	it("rejects http:// hostname-spoofing attempts", () => {
		// Full-hostname comparison prevents prefix/suffix attacks
		expect(() => validateLLMBaseUrl("http://localhost.evil.com/v1")).toThrow(
			"Insecure http://",
		);
		expect(() => validateLLMBaseUrl("http://127.0.0.1.evil.com/v1")).toThrow(
			"Insecure http://",
		);
		// Trailing dot — hostname 'localhost.' ≠ 'localhost'
		expect(() => validateLLMBaseUrl("http://localhost./v1")).toThrow(
			"Insecure http://",
		);
	});
	...
	it("rejects http:// non-loopback IPv6 addresses", () => {
		// Link-local IPv6
		expect(() => validateLLMBaseUrl("http://[fe80::1]/v1")).toThrow(
			"Insecure http://",
		);
		// IPv4-mapped IPv6 loopback — bracket-stripped to '::ffff:127.0.0.1' ≠ '::1'
		expect(() => validateLLMBaseUrl("http://[::ffff:127.0.0.1]/v1")).toThrow(
			"Insecure http://",
		);
	});

	it("rejects non-http schemes", () => {
		expect(() => validateLLMBaseUrl("file:///etc/passwd")).toThrow(
		...
		expect(() => validateLLMBaseUrl("ftp://example.com")).toThrow(
			"must use http:// or https://",
		);
	});
