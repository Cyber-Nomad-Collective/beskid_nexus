...
function makeGoFieldDecl(opts: {
	hasName: boolean;
	typeName: string;
}): MockNode {
	const nameNode = opts.hasName
		? makeNode("MyField", "field_identifier")
		: undefined;
...
function buildCaptureMap(
	entries: Record<string, MockNode | undefined>,
): CaptureMap {
...
describe("createHeritageExtractor", () => {
	...
	it("creates an extractor from a language enum (default config)", () => {
		...
		for (const lang of languages) {
			...
			expect(
				extractor.language,
				`${lang} extractor should have correct language`,
			).toBe(lang);
			expect(typeof extractor.extract).toBe("function");
...
describe("HeritageExtractor.extract", () => {
	...
	it("extracts trait-impl heritage", () => {
		...
		const result = rustExtractor.extract(
			captures,
			ctx("main.rs", SupportedLanguages.Rust),
		);
		expect(result).toEqual([
...
describe("Go HeritageExtractor — shouldSkipExtends", () => {
	...
	it("extracts anonymous struct embedding (no field name)", () => {
		...
		const result = extractor.extract(
			captures,
			ctx("main.go", SupportedLanguages.Go),
		);
		expect(result).toEqual([
...
describe("Ruby HeritageExtractor — call-based heritage", () => {
	...
	function makeCallNode(
		...
		const classNameNode = enclosingName
			? makeNode(enclosingName, "constant")
			: undefined;
...
describe("HeritageExtraction language configs", () => {
	...
	it("default language extractors have no custom hooks", () => {
		...
		for (const lang of defaultLanguages) {
			...
			expect(
				extractor.extractFromCall,
				`${lang} should not have extractFromCall`,
			).toBeUndefined();
		}
	});
});
...
describe("heritageExtractor on LanguageProvider", () => {
	it("all tree-sitter providers have heritageExtractor defined", () => {
		...
		for (const lang of languages) {
			...
			expect(
				provider.heritageExtractor,
				`${lang} should have a heritageExtractor`,
			).toBeDefined();
		}
	});
});
