...
vi.mock("../../src/core/git-staleness.js", () => ({
	...
	checkStalenessAsync: vi
		.fn()
		.mockResolvedValue({ isStale: false, commitsBehind: 0 }),
...
vi.mock("../../src/core/platform/capabilities.js", async (importOriginal) => {
	const actual =
		await importOriginal<
			typeof import("../../src/core/platform/capabilities.js")
		>();
	return {
		...actual,
		isVectorExtensionSupportedByPlatform:
			platformMocks.isVectorExtensionSupportedByPlatform,
	};
});

// Also mock the search modules to avoid loading onnxruntime
vi.mock("../../src/core/search/bm25-index.js", () => ({
	searchFTSFromLbug: vi
		.fn()
...
describe("LocalBackend.callTool", () => {
	...
	it("includes FTS-unavailable warning when ftsAvailable is false (#1403)", async () => {
		const { searchFTSFromLbug } = await import(
			"../../src/core/search/bm25-index.js"
		);
		vi
			.mocked(searchFTSFromLbug)
			.mockResolvedValueOnce({ results: [], ftsAvailable: false });
		(executeParameterized as any).mockResolvedValue([]);
	...
	it("does not include warning when ftsAvailable is true with zero results", async () => {
		...
		vi
			.mocked(searchFTSFromLbug)
			.mockResolvedValueOnce({ results: [], ftsAvailable: true });
		(executeParameterized as any).mockResolvedValue([]);
	...
	it("does not crash when searchFTSFromLbug throws (#1489)", async () => {
		...
		vi
			.mocked(searchFTSFromLbug)
			.mockRejectedValueOnce(new Error("bm25Results is not iterable"));
		(executeParameterized as any).mockResolvedValue([]);
	...
	it("skips vector index query when VECTOR is unsupported by the platform", async () => {
		...
		(executeQuery as any).mockImplementation(
			async (_repoId: string, cypher: string) => {
		...
		try {
			...
			expect(
				queries.some((cypher: string) => cypher.includes("QUERY_VECTOR_INDEX")),
			).toBe(false);
			expect(
				queries.some(
					(cypher: string) =>
