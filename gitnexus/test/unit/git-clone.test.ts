...
describe("git-clone", () => {
	describe("extractRepoName", () => {
		...
		it("strips leading dashes to prevent argument injection", () => {
			expect(
				extractRepoName("https://github.com/user/--upload-pack=payload.git"),
			).toBe("upload-pack_payload");
		...
		it("sanitizes unsafe directory characters", () => {
			...
			expect(extractRepoName("https://github.com/user/repo<tag>.git")).toBe(
				"repo_tag_",
			);
		});
		...
		it("sanitizes whitespace and backslashes", () => {
			expect(extractRepoName("https://example.com/foo:repo name")).toBe(
				"repo_name",
			);
	...
	describe("getCloneDir", () => {
		...
		it("returned path is always a direct child of the clone root", () => {
			const cloneRoot = path.resolve(
				path.join(os.homedir(), ".gitnexus", "repos"),
			);
			const dir = getCloneDir("my-repo");
	...
	describe("validateGitUrl", () => {
		...
		it("allows valid HTTP URLs", () => {
			expect(() =>
				validateGitUrl("http://gitlab.com/user/repo.git"),
			).not.toThrow();
		});
		...
		it("blocks IPv4 loopback", () => {
			expect(() => validateGitUrl("http://127.0.0.1/repo.git")).toThrow(
				"private/internal",
			);
			expect(() => validateGitUrl("http://127.255.0.1/repo.git")).toThrow(
				"private/internal",
			);
		});
		...
		it("blocks IPv6 loopback ::1", () => {
			...
			expect(() => validateGitUrl("http://[::1]/repo.git")).toThrow(
				"private/internal",
			);
		});

		it("blocks IPv4 private ranges (10.x, 172.16-31.x, 192.168.x)", () => {
			expect(() => validateGitUrl("http://10.0.0.1/repo.git")).toThrow(
				"private/internal",
			);
			expect(() => validateGitUrl("http://172.16.0.1/repo.git")).toThrow(
				"private/internal",
			);
			expect(() => validateGitUrl("http://172.31.255.255/repo.git")).toThrow(
				"private/internal",
			);
			expect(() => validateGitUrl("http://192.168.1.1/repo.git")).toThrow(
				"private/internal",
			);
		});

		it("blocks link-local addresses", () => {
			expect(() => validateGitUrl("http://169.254.1.1/repo.git")).toThrow(
				"private/internal",
			);
		});
		...
		it("blocks IPv6 ULA (fc/fd)", () => {
			expect(() => validateGitUrl("http://[fc00::1]/repo.git")).toThrow(
				"private/internal",
			);
			expect(() => validateGitUrl("http://[fd12::1]/repo.git")).toThrow(
				"private/internal",
			);
		});

		it("blocks IPv6 link-local (fe80)", () => {
			expect(() => validateGitUrl("http://[fe80::1]/repo.git")).toThrow(
				"private/internal",
			);
		});
		...
		it("blocks IPv4-compatible IPv6 (RFC 4291 deprecated, ::w.x.y.z)", () => {
			...
			expect(() => validateGitUrl("http://[::7f00:1]/repo.git")).toThrow(
				"private/internal",
			);
			// 169.254.169.254 (cloud metadata) embedded as IPv4-compatible
			expect(() => validateGitUrl("http://[::a9fe:a9fe]/repo.git")).toThrow(
				"private/internal",
			);
		});
		...
		it("blocks IPv4-compatible IPv6 in expanded / zero-padded forms", () => {
			...
			expect(() =>
				validateGitUrl("http://[0:0:0:0:0:0:127.0.0.1]/repo.git"),
			).toThrow("private/internal");
		});

		it("blocks NAT64 well-known prefix (64:ff9b::/96)", () => {
			// 64:ff9b::7f00:1 → 127.0.0.1 via NAT64 translation
			expect(() => validateGitUrl("http://[64:ff9b::7f00:1]/repo.git")).toThrow(
				"private/internal",
			);
			...
			expect(() => validateGitUrl("http://[64:ff9b:1::1]/repo.git")).toThrow(
				"private/internal",
			);
		});
		...
		it("blocks NAT64 with embedded RFC1918 addresses", () => {
			...
			expect(() => validateGitUrl("http://[64:ff9b::a00:1]/repo.git")).toThrow(
				"private/internal",
			); // 10.0.0.1
		...
		it("blocks CGN range (100.64.0.0/10)", () => {
			expect(() => validateGitUrl("http://100.64.0.1/repo.git")).toThrow(
				"private/internal",
			);
			expect(() => validateGitUrl("http://100.127.255.255/repo.git")).toThrow(
				"private/internal",
			);
		});

		it("blocks benchmarking range (198.18.0.0/15)", () => {
			expect(() => validateGitUrl("http://198.18.0.1/repo.git")).toThrow(
				"private/internal",
			);
			expect(() => validateGitUrl("http://198.19.255.255/repo.git")).toThrow(
				"private/internal",
			);
		});

		it("blocks numeric decimal IP encoding", () => {
			expect(() => validateGitUrl("http://2130706433/repo.git")).toThrow(
				"private/internal",
			);
		});

		it("blocks hex IP encoding", () => {
			expect(() => validateGitUrl("http://0x7f000001/repo.git")).toThrow(
				"private/internal",
			);
		});

		it("blocks 0.0.0.0", () => {
			expect(() => validateGitUrl("http://0.0.0.0/repo.git")).toThrow(
				"private/internal",
			);
		});
	});
	...
	describe("cloneOrPull — containment barrier", () => {
		...
		it("rejects URLs that fail validateGitUrl even when the target shape is valid", async () => {
			...
			await expect(
				cloneOrPull("http://127.0.0.1/repo.git", fakeTarget),
			).rejects.toThrow("private/internal");
			await expect(
				cloneOrPull("http://localhost/repo.git", fakeTarget),
			).rejects.toThrow("private/internal");
	...
	describe("normalizeGitUrlForCompare", () => {
		...
		it("preserves non-default ports", () => {
			expect(
				normalizeGitUrlForCompare("https://git.corp:8443/owner/repo"),
			).not.toBe(normalizeGitUrlForCompare("https://git.corp/owner/repo"));
		});

		it("strips userinfo (basic auth) so equivalent URLs compare equal", () => {
			expect(
				normalizeGitUrlForCompare("https://user:pass@github.com/owner/repo.git"),
			).toBe(normalizeGitUrlForCompare("https://github.com/owner/repo"));
		});
	...
	describe("assertRemoteMatchesRequestedUrl", () => {
			...
			await new Promise<void>((resolve, reject) => {
				const proc = spawn(
					...
					[
						"config",
						"remote.origin.url",
						"https://github.com/legitorg/myproject.git",
					],
		...
		it("accepts equivalent forms (with/without .git, trailing slash, default port)", async () => {
			await expect(
				assertRemoteMatchesRequestedUrl(
					fixtureDir,
					"https://github.com/legitorg/myproject",
				),
			).resolves.toBeUndefined();
			await expect(
				assertRemoteMatchesRequestedUrl(
		...
		it("rejects a different owner on the same host", async () => {
			await expect(
				assertRemoteMatchesRequestedUrl(
					fixtureDir,
					"https://github.com/attacker/myproject.git",
				),
			).rejects.toThrow("not the requested URL");
		});

		it("rejects when the directory has no remote.origin", async () => {
			const noRemoteDir = await fs.mkdtemp(
				path.join(os.tmpdir(), "gitnexus-no-remote-"),
			);
			try {
				await new Promise<void>((resolve, reject) => {
					const proc = spawn("git", ["init", "--quiet"], {
