...
describe("envVarNameFor", () => {
	it("produces upper-cased env-var names from the enum value", () => {
		expect(envVarNameFor(SupportedLanguages.Python)).toBe(
			"REGISTRY_PRIMARY_PYTHON",
		);
		expect(envVarNameFor(SupportedLanguages.TypeScript)).toBe(
			"REGISTRY_PRIMARY_TYPESCRIPT",
		);
		expect(envVarNameFor(SupportedLanguages.JavaScript)).toBe(
			"REGISTRY_PRIMARY_JAVASCRIPT",
		);
	});
	...
	it("uses the enum VALUE, not the key, for languages whose key differs from the value", () => {
		...
		expect(envVarNameFor(SupportedLanguages.CPlusPlus)).toBe(
			"REGISTRY_PRIMARY_CPP",
		);
		expect(envVarNameFor(SupportedLanguages.CSharp)).toBe(
			"REGISTRY_PRIMARY_CSHARP",
		);
	});
