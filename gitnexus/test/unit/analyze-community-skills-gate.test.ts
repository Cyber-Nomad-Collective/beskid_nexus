...
describe("shouldGenerateCommunitySkillFiles (#742 / PR 1485)", () => {
	...
	it("is false when pipelineResult is missing", () => {
		expect(
			shouldGenerateCommunitySkillFiles({ skills: true, indexOnly: false }, null),
		).toBe(false);
		expect(shouldGenerateCommunitySkillFiles({ skills: true }, undefined)).toBe(
			false,
		);
	});
	...
	it("is true when --skills is set, pipeline exists, and not index-only", () => {
		...
		expect(shouldGenerateCommunitySkillFiles({ skills: true }, { x: 1 })).toBe(
			true,
		);
	});
	...
	it("is false when --skills is omitted", () => {
		expect(
			shouldGenerateCommunitySkillFiles({ indexOnly: false }, { x: 1 }),
		).toBe(false);
