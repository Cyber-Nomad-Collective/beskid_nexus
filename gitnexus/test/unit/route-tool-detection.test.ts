...
describe("nextjsFileToRouteURL", () => {
	it("extracts App Router API routes", () => {
		...
		expect(nextjsFileToRouteURL("app/api/auth/login/route.tsx")).toBe(
			"/api/auth/login",
		);
	});
	...
	it("handles dynamic segments", () => {
		expect(
			nextjsFileToRouteURL("app/api/organizations/[slug]/grants/route.ts"),
		).toBe("/api/organizations/[slug]/grants");
		expect(nextjsFileToRouteURL("app/api/users/[id]/route.ts")).toBe(
			"/api/users/[id]",
		);
	});
	...
	it("strips route groups from App Router paths", () => {
		expect(nextjsFileToRouteURL("app/(admin)/api/users/route.ts")).toBe(
			"/api/users",
		);
		expect(nextjsFileToRouteURL("app/(marketing)/api/newsletter/route.ts")).toBe(
			"/api/newsletter",
		);
	});

	it("extracts Pages Router API routes", () => {
		expect(nextjsFileToRouteURL("pages/api/auth/login.ts")).toBe(
			"/api/auth/login",
		);
	...
	it("handles Windows-style backslash paths", () => {
		expect(nextjsFileToRouteURL("app\\api\\grants\\route.ts")).toBe(
			"/api/grants",
		);
	});
});
...
describe("routeMatches", () => {
	...
	it("matches dynamic segments on either side", () => {
		...
		expect(
			routeMatches("/api/orgs/[param]/grants", "/api/orgs/[slug]/grants"),
		).toBe(true);
	});
...
describe("detectStatusCode", () => {
	...
	it("detects status in second arg with extra properties", () => {
		const content =
			'return NextResponse.json({ error: "fail" }, { status: 422, headers: {} })';
		const jsonPos = content.indexOf(".json");
		const firstArgClose = content.indexOf("}");
...
describe("middlewareMatcherMatchesRoute", () => {
	it("matches prefix pattern with :path*", () => {
		...
		expect(middlewareMatcherMatchesRoute("/api/:path*", "/api/auth/login")).toBe(
			true,
		);
	...
	it("does not match unrelated routes with :path*", () => {
		expect(middlewareMatcherMatchesRoute("/dashboard/:path*", "/api/users")).toBe(
			false,
		);
	...
	it("matches exact route", () => {
		...
		expect(middlewareMatcherMatchesRoute("/login", "/login/callback")).toBe(
			false,
		);
	});
	...
	it("matches regex-style negative lookahead pattern", () => {
		...
		expect(middlewareMatcherMatchesRoute(matcher, "/settings/profile")).toBe(
			true,
		);
