...
} from "../../src/core/ingestion/import-resolvers/configs/csharp.js";
import {
	dartImportConfig,
	dartPackageStrategy,
...
import {
	goImportConfig,
	goPackageStrategy,
} from "../../src/core/ingestion/import-resolvers/configs/go.js";
import {
	javaImportConfig,
	javaJvmStrategy,
	kotlinImportConfig,
...
import {
	phpImportConfig,
	phpPsr4Strategy,
} from "../../src/core/ingestion/import-resolvers/configs/php.js";
import {
	pythonImportConfig,
	pythonImportStrategy,
} from "../../src/core/ingestion/import-resolvers/configs/python.js";
import {
	rubyImportConfig,
	rubyRequireStrategy,
} from "../../src/core/ingestion/import-resolvers/configs/ruby.js";
import {
	rustImportConfig,
	rustModuleStrategy,
} from "../../src/core/ingestion/import-resolvers/configs/rust.js";
import {
	swiftImportConfig,
	swiftPackageStrategy,
} from "../../src/core/ingestion/import-resolvers/configs/swift.js";
...
function makeCtx(
	files: string[],
	overrides: Partial<ResolveCtx["configs"]> = {},
): ResolveCtx {
...
describe("createImportResolver", () => {
	...
	it("returns first non-null result from strategy chain", () => {
		...
		const strategyB: ImportResolverStrategy = () => ({
			kind: "files",
			files: ["b.ts"],
		});
		const strategyC: ImportResolverStrategy = () => ({
			kind: "files",
			files: ["c.ts"],
		});
...
describe("goPackageStrategy", () => {
	it("resolves go.mod package imports to a package result with dirSuffix", () => {
		...
		const result = goPackageStrategy(
...
describe("javaJvmStrategy", () => {
	...
	it("resolves member imports to the owning class file", () => {
		...
		const result = javaJvmStrategy(
			"com.example.Constants.VALUE",
			"App.java",
			ctx,
		);
		expect(result).toEqual({ kind: "files", files });
...
describe("kotlinJvmStrategy", () => {
	...
	it("resolves member imports to the owning class file", () => {
		...
		const ctx = makeCtx(files);
		const result = kotlinJvmStrategy(
			"com.example.Constants.VALUE",
			"App.kt",
			ctx,
		);
...
describe("rustModuleStrategy", () => {
	it("resolves scoped grouped imports to per-item files", () => {
		...
		const result = rustModuleStrategy(
...
describe("csharpNamespaceStrategy", () => {
	it("resolves namespace imports via .csproj root-namespace mapping", () => {
		const files = [
			...
			"src/Services/Auth/TokenService.cs",
		];
		const ctx = makeCtx(files, {
...
describe("pythonImportStrategy", () => {
	...
	it("absorbs unresolved nested external dotted imports like django.urls and django.core.*", () => {
		...
		expect(
			pythonImportStrategy("django.core.asgi", "config/asgi.py", ctx),
		).toEqual({
			kind: "files",
			files: [],
		});
		expect(
			pythonImportStrategy("django.core.wsgi", "config/wsgi.py", ctx),
		).toEqual({
			kind: "files",
			files: [],
		});
	...
	it("keeps dotted internal imports unresolved here when the leading package exists in-repo", () => {
		...
		const result = pythonImportStrategy(
			"accounts.models",
			"billing/models.py",
			ctx,
		);
