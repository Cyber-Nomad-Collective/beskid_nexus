...
beforeAll(() => {
	...
	guardTmpDir = fs.mkdtempSync(
		path.join(os.tmpdir(), "gitnexus-cursor-hook-guard-"),
	);
	...
	it("does not pass shell: true to spawnSync", () => {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (/shell:\s*(true|isWin)/.test(line)) {
				throw new Error(
					`Cursor hook line ${i + 1} has shell injection risk: ${line.trim()}`,
				);
			}
		}
	});
...
describe("Cursor hook concurrency guard", () => {
	...
	it("uses atomic fixed-name slot files (hard cap, not soft TOCTOU cap)", () => {
		...
		const slotFn = lockSource.slice(
			...
			lockSource.indexOf(
				"function",
				lockSource.indexOf("function acquireHookSlot") + 1,
			),
		);
