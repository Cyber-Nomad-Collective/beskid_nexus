...
describe("isNetworkFetchError", () => {
	...
	it("returns true for ECONNREFUSED", () => {
		expect(isNetworkFetchError("connect ECONNREFUSED 13.45.67.89:443")).toBe(
			true,
		);
	});

	it("returns true for ENOTFOUND (DNS failure)", () => {
		expect(isNetworkFetchError("getaddrinfo ENOTFOUND huggingface.co")).toBe(
			true,
		);
	});
	...
	it("returns false for module-not-found errors", () => {
		expect(isNetworkFetchError("Cannot find module onnxruntime-node")).toBe(
			false,
		);
	});
});
...
describe("isHfCircuitOpenError", () => {
	it("returns true for a circuit-open tag message", () => {
		expect(isHfCircuitOpenError(`${CIRCUIT_OPEN_TAG}: circuit is open`)).toBe(
			true,
		);
	});
...
describe("withHfDownloadRetry", () => {
	...
	it("does not retry non-network errors", async () => {
		const fn = vi
			.fn()
			.mockRejectedValue(new Error("Failed to initialize CUDA backend"));
		const cb = new CircuitBreaker();
		await expect(
			withHfDownloadRetry(fn, { circuit: cb, maxAttempts: 3, baseDelayMs: 0 }),
	...
	it("fails immediately when the circuit is already open", async () => {
		...
		await expect(withHfDownloadRetry(fn, { circuit: cb })).rejects.toThrow(
			CIRCUIT_OPEN_TAG,
		);
...
describe("withHfDownloadRetry env overrides", () => {
	...
	it("HF_MAX_ATTEMPTS=1 gives exactly 1 attempt", async () => {
		...
		await expect(
			withHfDownloadRetry(fn, { circuit: cb, baseDelayMs: 0 }),
		).rejects.toThrow("ECONNREFUSED");
		expect(fn).toHaveBeenCalledTimes(1);
	});
	...
	it("HF_DOWNLOAD_TIMEOUT_MS is used as the per-attempt timeout when valid", async () => {
		...
		try {
			...
			const neverResolves = () => new Promise<never>(() => {});
			const cb = new CircuitBreaker({ failureThreshold: 99 });
			const promise = withHfDownloadRetry(neverResolves, {
				circuit: cb,
				maxAttempts: 1,
			});
