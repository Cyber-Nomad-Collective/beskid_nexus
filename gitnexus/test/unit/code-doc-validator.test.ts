...
import {
	containsSpecProse,
	validateSpecLinks,
} from "../../src/server/nexus/code-doc-validator.js";
...
describe("containsSpecProse", () => {
	it("flags codeDoc that copies spec index excerpt", () => {
		...
		expect(
			containsSpecProse(`This module handles ${specExcerpt} in the runtime.`, [
				specExcerpt,
			]),
		).toBe(true);
	});
	it("allows codeDoc that only references spec in links", () => {
		expect(
			containsSpecProse(
				"Indexes symbols via tree-sitter and stores call edges.",
				[],
			),
		).toBe(false);
	});
