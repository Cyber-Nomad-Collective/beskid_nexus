...
describe("GroupService — missing group error handling", () => {
	...
	let service: InstanceType<
		typeof import("../../src/core/group/service.js").GroupService
	>;
	...
	beforeEach(async () => {
		...
		({ GroupNotFoundError } = await import(
			"../../src/core/group/config-parser.js"
		));
	...
	it("groupSync re-throws non-ENOENT errors", async () => {
		...
		await expect(service.groupSync({ name: "bad-yaml" })).rejects.toThrow(
			"YAML parse error",
		);
	});
	...
	it("groupQuery re-throws non-ENOENT errors", async () => {
		loadGroupConfigMock.mockRejectedValue(new Error("YAML parse error"));
		await expect(
			service.groupQuery({ name: "bad-yaml", query: "auth" }),
		).rejects.toThrow("YAML parse error");
	});

	it("groupStatus re-throws non-ENOENT errors", async () => {
		loadGroupConfigMock.mockRejectedValue(new Error("YAML parse error"));
		await expect(service.groupStatus({ name: "bad-yaml" })).rejects.toThrow(
			"YAML parse error",
		);
	});
