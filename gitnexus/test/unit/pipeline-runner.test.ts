...
describe("runPipeline", () => {
	...
	it("rejects missing dependencies", async () => {
		...
		await expect(runPipeline([phase], makeCtx())).rejects.toThrow(
	...
	it("emits a terminal 'error' progress event exactly once on phase failure", async () => {
		...
		await expect(runPipeline([phase], ctx)).rejects.toThrow(
			/Phase 'failing' failed/,
		);
...
describe("getPhaseOutput", () => {
	...
	it("throws for missing phase", () => {
		...
		expect(() => getPhaseOutput(deps, "missing")).toThrow(
