...
describe("typescriptProvider.mergeBindings adapter", () => {
	...
	it("is scope-id independent because finalize calls it per (scope, name)", () => {
		...
		if (merge === undefined)
			throw new Error("typescriptProvider.mergeBindings missing");
...
describe("emitTsScopeCaptures — arity metadata integration", () => {
	it("attaches @declaration.parameter-count to function declarations", () => {
		...
		expect(
			(fn as Record<string, { text: string }>)["@declaration.parameter-count"]
				.text,
		).toBe("3");
		expect(
			(fn as Record<string, { text: string }>)[
	...
	it("omits parameter-count when rest params make max unknown", () => {
		...
		expect(fn).toBeDefined();
		expect(
			(fn as Record<string, unknown>)["@declaration.parameter-count"],
		).toBeUndefined();
		expect(
			(fn as Record<string, { text: string }>)["@declaration.parameter-types"]
				.text,
		).toContain("params");
	});
	...
	it("attaches @reference.arity on free calls", () => {
		...
		expect(
			(call as Record<string, { text: string }>)["@reference.arity"].text,
		).toBe("3");
	});
	...
	it("attaches @reference.arity on constructor calls", () => {
		...
		const call = caps.find(
			(c) =>
				(c as Record<string, unknown>)["@reference.call.constructor"] !== undefined,
		);
		expect(call).toBeDefined();
		expect(
			(call as Record<string, { text: string }>)["@reference.arity"].text,
		...
		const types = JSON.parse(
			(call as Record<string, { text: string }>)["@reference.parameter-types"]
				.text,
		) as string[];
	...
	it("attaches @reference.arity on member calls", () => {
		...
		const call = caps.find(
			(c) =>
				(c as Record<string, unknown>)["@reference.call.member"] !== undefined,
		);
		expect(call).toBeDefined();
		expect(
			(call as Record<string, { text: string }>)["@reference.arity"].text,
