	...
	const finalized = finalizeScopeModel(parsedFiles, {
		hooks: {
			resolveImportTarget: (targetRaw, fromFile) =>
				typescriptScopeResolver.resolveImportTarget(
					targetRaw,
					fromFile,
					allFilePaths,
				),
...
function moduleTypeBinding(
	...
	if (moduleScope === undefined)
		throw new Error(`no module scope for ${filePath}`);
...
`,
	...
	it("mirrors import return types from bindingAugmentations-only refs", () => {
		...
		const sourceModule = {
			...
			bindings: new Map(),
			typeBindings: new Map([
				["getUser", { rawName: "User", source: "return-annotation" }],
			]),
		} as unknown as Scope;
		const importedDef = {
		...
		const indexes = {
			...
			bindingAugmentations: new Map([
				[
					appScopeId,
					new Map([
						["getUser", [{ def: importedDef, origin: "import" } as BindingRef]],
					]),
				],
			]),
			sccs: [{ files: ["app.ts"] }],
