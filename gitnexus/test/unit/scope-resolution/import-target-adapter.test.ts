...
function workspace(
	...
	for (const [lang, resolver] of entries)
		providers.set(lang, fakeProvider(resolver));
...
describe("buildImportTargetWorkspace", () => {
	...
	it("threads the shared ResolveCtx into every entry", () => {
		...
		const tsResolver: ImportResolverFn = () => ({
			kind: "files",
			files: ["x.ts"],
		});
		const ws = workspace([
...
describe("resolveImportTargetAcrossLanguages", () => {
	it("dispatches to the resolver for the fromFile extension", () => {
		...
		const ws = workspace([[SupportedLanguages.Python, pyResolver]]);
		const result = resolveImportTargetAcrossLanguages(
			"models.user",
			"src/app.py",
			ws,
		);
	...
	it("routes to different resolvers based on the fromFile extension", () => {
		...
		expect(resolveImportTargetAcrossLanguages("x", "a.py", ws)).toBe(
			"resolved.py",
		);
		expect(resolveImportTargetAcrossLanguages("x", "a.ts", ws)).toBe(
			"resolved.ts",
		);
	});
	...
	it("returns null when the resolver returns null", () => {
		...
		const ws = workspace([[SupportedLanguages.Python, pyResolver]]);
		expect(
			resolveImportTargetAcrossLanguages("external_pkg", "app.py", ws),
		).toBeNull();
	});
	...
	it("takes the first file from a package-kind result", () => {
		...
		expect(resolveImportTargetAcrossLanguages("pkg", "app.py", ws)).toBe(
			"pkg/index.py",
		);
	});
	...
	it("returns null when workspaceIndex is undefined / malformed", () => {
		...
		expect(
			resolveImportTargetAcrossLanguages("x", "a.py", {} as unknown),
		).toBeNull();
	});
