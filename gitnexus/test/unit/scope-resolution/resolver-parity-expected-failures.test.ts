...
describe("resolver parity expected legacy failures", () => {
	it("uses the same env var convention as the parity workflow", () => {
		...
		expect(resolverParityFlagName("c-plus-plus")).toBe(
			"REGISTRY_PRIMARY_C_PLUS_PLUS",
		);
	});

	it("recognizes only legacy parity runs", () => {
		expect(
			isLegacyResolverParityRun("csharp", { REGISTRY_PRIMARY_CSHARP: "0" }),
		).toBe(true);
		expect(
			isLegacyResolverParityRun("csharp", { REGISTRY_PRIMARY_CSHARP: "false" }),
		).toBe(true);
		expect(
			isLegacyResolverParityRun("csharp", { REGISTRY_PRIMARY_CSHARP: "1" }),
		).toBe(false);
