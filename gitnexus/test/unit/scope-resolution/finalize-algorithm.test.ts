...
const defaultHooks = (files: readonly FinalizeFile[]): FinalizeHooks => ({
	...
	expandsWildcardTo(targetModuleScope) {
		...
		return target.localDefs
			.map((d) => deriveSimple(d))
...
const wildcard = (targetRaw: string): ParsedImport => ({
	kind: "wildcard",
	targetRaw,
});
...
const sideEffect = (targetRaw: string): ParsedImport => ({
	kind: "side-effect",
	targetRaw,
});
...
describe("finalize", () => {
	describe("trivial / acyclic", () => {
		...
		it("marks an edge unresolved when target file cannot be resolved", () => {
			...
			const out = finalize(
				{ files: [a], workspaceIndex: undefined },
				defaultHooks([a]),
			);
			const edge = firstImport(out, a.moduleScope)!;
		...
		it("links dynamic-resolved imports as file-level edges without bindings", () => {
			const feature = file("feature", [
				def("def:feature.Feature", "Class", "feature.Feature"),
			]);
	...
	describe("cycles + bounded fixpoint", () => {
		...
		it("separates disjoint SCCs", () => {
			...
			const c = file(
				"c",
				[def("def:c.P", "Class", "c.P")],
				[named("Q", "Q", "d")],
			);
			const d = file(
				"d",
				[def("def:d.Q", "Class", "d.Q")],
				[named("P", "P", "c")],
			);
			const files = [a, b, c, d];
			const out = finalize(
	...
	describe("wildcard expansion", () => {
		it("expands `wildcard` into one ImportEdge per exported name", () => {
			...
			expect(new Set(edges.map((e) => e.localName))).toEqual(
				new Set(["X", "Y", "Z"]),
			);
	...
	describe("module-scope binding materialization", () => {
		...
		it("resolves imported defs across many files via O(1) defById index lookup", () => {
			...
			for (let i = 0; i < N; i++) {
				...
				leafFiles.push(
					file(fp, [def(`def:${fp}.${localName}`, "Class", `${fp}.${localName}`)]),
				);
	...
	describe("SCC-DAG exposure for parallelism", () => {
		it("returns SCCs in reverse-topological order (leaves first)", () => {
			...
			const b = file(
				"b",
				[def("def:b.B", "Class", "b.B")],
				[named("C", "C", "c")],
			);
			const a = file(
				"a",
				[def("def:a.A", "Class", "a.A")],
				[named("B", "B", "b")],
			);
			const files = [a, b, c];
			const out = finalize(
