...
const ref = (
	nodeId: string,
	origin: BindingRef["origin"] = "local",
): BindingRef => ({ def: def(nodeId), origin }) as BindingRef;
...
function indexesWith({
	...
	if (augmented !== undefined)
		bindingAugmentations.set(SCOPE, new Map([["name", augmented]]));
...
describe("walker helpers read bindingAugmentations", () => {
	it("findClassBindingInScope finds class-like refs that exist only in augmentations", () => {
		...
		const indexes = indexesForScopeLookup(
			moduleScope,
			new Map([["ClassA", [classRef]]]),
		);

		expect(findClassBindingInScope(SCOPE, "ClassA", indexes)?.nodeId).toBe(
			"ClassA",
		);
	});
	...
	it("findCallableBindingInScope finds callable refs that exist only in augmentations", () => {
		...
		const indexes = indexesForScopeLookup(
			moduleScope,
			new Map([["callMe", [callableRef]]]),
		);

		expect(findCallableBindingInScope(SCOPE, "callMe", indexes)?.nodeId).toBe(
			"callMe",
		);
	});
	...
	it("findExportedDefByName finds callable refs that exist only in augmentations", () => {
		const moduleScope = scope(SCOPE);
		const callableRef = {
			def: def("fromAugmentation"),
			origin: "import",
		} as BindingRef;
		const indexes = indexesForScopeLookup(
			moduleScope,
			new Map([["run", [callableRef]]]),
		);
		...
		expect(
			findExportedDefByName("run", SCOPE, indexes, workspaceIndex)?.nodeId,
		).toBe("fromAugmentation");
	});
});
