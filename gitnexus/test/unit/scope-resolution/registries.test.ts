...
const mkDef = (
	overrides: Partial<SymbolDefinition> & { nodeId: string },
): SymbolDefinition => ({
...
const evidenceOfKind = (res: Resolution, kind: string) =>
	res.evidence.find((e) => e.kind === kind);
...
describe("Step 1: lexical scope-chain walk", () => {
	...
	it("emits origin=import evidence when the binding is imported", () => {
		...
		expect(evidenceOfKind(res[0]!, "import")?.weight).toBe(
			EvidenceWeights.import,
		);
	});
});
...
describe("Step 5: arity filter", () => {
	...
	it("keeps incompatible candidates when at least one verdict is unknown (soft penalty)", () => {
		...
		const mod = mkScope({
			...
			parent: null,
			bindings: {
				save: [mkBinding(save3, "local"), mkBinding(saveUnknown, "local")],
			},
		});
		const ctx = makeCtx([mod], [save3, saveUnknown], {
			arity: (_callsite, def) =>
				def.nodeId === "def:save-unknown" ? "unknown" : "incompatible",
		});
		const results = buildMethodRegistry(ctx).lookup("save", "scope:m", {
			callsite: { arity: 1 },
		});
...
describe("Step 7: tie-break cascade", () => {
	...
	it("orders multiple same-scope candidates by confidence DESC", () => {
		...
		const mod = mkScope({
			...
			bindings: {
				User: [
					mkBinding(wildcardClass, "wildcard"),
					mkBinding(localClass, "local"),
				],
			},
		});
...
describe("lookupQualified (§4.5)", () => {
	...
	it("returns empty for unknown qualified names", () => {
		...
		expect(
			lookupQualified("app.Ghost", { acceptedKinds: ["Class"] }, ctx),
		).toEqual([]);
	});
...
describe("Step 3: owner-scoped contributor", () => {
	it("merges contributor hits as origin=local at the receiver scope", () => {
		...
		expect(evidenceOfKind(results[0]!, "owner-match")?.weight).toBe(
			EvidenceWeights.ownerMatch,
		);
	});
});
...
describe("Step 2: type-binding + MRO walk", () => {
	...
	it("walks up the MRO when the method is declared on an ancestor", () => {
		const baseClass = mkDef({
			nodeId: "def:Base",
			type: "Class",
			qualifiedName: "Base",
		});
		const derivedClass = mkDef({
			nodeId: "def:Derived",
			type: "Class",
