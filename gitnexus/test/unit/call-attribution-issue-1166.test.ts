 ...
 */
function attributeCall(callNode: SyntaxNode): {
	name: string | null;
	...
	while (current) {
		if (FUNCTION_NODE_TYPES.has(current.type)) {
			const efn =
				typescriptProvider.methodExtractor?.extractFunctionName?.(current);
...
describe("issue #1166 — Bug A: anonymous arrows do not borrow parameter names", () => {
	...
	it("returns null for function_expression (would otherwise leak named-funexpr identifier)", () => {
		...
		const tree = parser.parse(
			"const x = arr.filter(function (x) { return isOk(x); });",
		);
    ...
    `);
		...
		for (const s of sites) {
			expect(s.attributedTo, `call ${s.calledName} at L${s.line}`).not.toBe(
				"file",
			);
		}
	});
...
describe("issue #1166 — regression guards", () => {
    ...
    `);
		expect(findCall(sites, "sharedValidateFile")?.attributedTo).toBe(
			"validateFile",
		);
