...
describe("preprocessImportPath", () => {
	describe("quote and bracket stripping", () => {
		it("strips double quotes from a bare module path", () => {
			...
			expect(
				preprocessImportPath(
					'"foo"',
					node,
					getProvider(SupportedLanguages.TypeScript),
				),
			).toBe("foo");
		});
		...
		it("strips single quotes from a bare module path", () => {
			...
			expect(
				preprocessImportPath(
					"'bar/baz'",
					node,
					getProvider(SupportedLanguages.JavaScript),
				),
			).toBe("bar/baz");
		});
		...
		it("strips angle brackets from a C-style include path", () => {
			const node = makeNode();
			expect(
				preprocessImportPath("<stdio.h>", node, getProvider(SupportedLanguages.C)),
			).toBe("stdio.h");
		});
	...
	describe("null returns for invalid inputs", () => {
		...
		it("returns null for a string containing control characters", () => {
			...
			expect(
				preprocessImportPath(
					"foo\x01bar",
					node,
					getProvider(SupportedLanguages.Rust),
				),
			).toBeNull();
		});
		...
		it("returns null for a string containing a null byte", () => {
			...
			expect(
				preprocessImportPath(
					"foo\x00bar",
					node,
					getProvider(SupportedLanguages.Go),
				),
			).toBeNull();
		});
		...
		it("returns null for a path exceeding 2048 characters", () => {
			...
			expect(
				preprocessImportPath(
					longPath,
					node,
					getProvider(SupportedLanguages.Python),
				),
			).toBeNull();
		});
		...
		it("accepts a path of exactly 2048 characters", () => {
			...
			expect(
				preprocessImportPath(maxPath, node, getProvider(SupportedLanguages.Python)),
			).toBe(maxPath);
		});
	});
