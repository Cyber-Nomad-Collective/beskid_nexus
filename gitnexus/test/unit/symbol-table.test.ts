...
import type { SymbolTableWriter } from "../../src/core/ingestion/model/symbol-table.js";
...
describe("SymbolTable", () => {
	...
	describe("lookupFieldByOwner", () => {
		...
		it("returns undefined for unknown owner", () => {
			...
			expect(
				model.fields.lookupFieldByOwner("class:Unknown", "address"),
			).toBeUndefined();
		});
		...
		it("returns undefined for unknown field name", () => {
			...
			expect(
				model.fields.lookupFieldByOwner("class:User", "email"),
			).toBeUndefined();
		});

		it("returns undefined for empty table", () => {
			expect(
				model.fields.lookupFieldByOwner("class:User", "name"),
			).toBeUndefined();
		});
	...
	describe("lookupMethodByOwner", () => {
		...
		it("returns undefined for unknown owner", () => {
			...
			expect(
				model.methods.lookupMethodByOwner("class:Unknown", "save"),
			).toBeUndefined();
		});
		...
		it("returns undefined for unknown method name", () => {
			...
			expect(
				model.methods.lookupMethodByOwner("class:User", "delete"),
			).toBeUndefined();
		});

		it("returns undefined for empty table", () => {
			expect(
				model.methods.lookupMethodByOwner("class:User", "save"),
			).toBeUndefined();
		});
		...
		it("returns undefined for overloads both missing returnType (ambiguous)", () => {
			...
			expect(
				model.methods.lookupMethodByOwner("class:Handler", "process"),
			).toBeUndefined();
		});
		...
		it("returns undefined for overloads with different returnTypes (ambiguous)", () => {
			...
			expect(
				model.methods.lookupMethodByOwner("class:Converter", "convert"),
			).toBeUndefined();
		});
	...
	describe("clear", () => {
		it("resets all state including fieldByOwner, methodByOwner, and classByName", () => {
			...
			expect(
				model.fields.lookupFieldByOwner("class:User", "address"),
			).toBeUndefined();
			expect(
				model.methods.lookupMethodByOwner("class:User", "save"),
			).toBeUndefined();
	...
	describe("lookupClassByQualifiedName", () => {
		...
		it("after clear(), returns empty array", () => {
			...
			expect(model.types.lookupClassByQualifiedName("Services.User")).toHaveLength(
				1,
			);
			model.clear();
			expect(model.types.lookupClassByQualifiedName("Services.User")).toEqual([]);
	...
	describe("SemanticModel container (SM-21 inversion)", () => {
		...
		it("feeding a Property with ownerId populates model.fields", () => {
			...
			expect(model.fields.lookupFieldByOwner("class:User", "name")?.nodeId).toBe(
				"prop:User.name",
			);
		});
		...
		it("arity filtering disambiguates overloads via model.methods", () => {
			...
			expect(
				model.methods.lookupMethodByOwner("class:User", "greet", 0)?.nodeId,
			).toBe("mtd:greet:0");
			expect(
				model.methods.lookupMethodByOwner("class:User", "greet", 1)?.nodeId,
			).toBe("mtd:greet:1");
		});
		...
		it("silently skips Property without ownerId (no model.fields registration)", () => {
			...
			expect(model.symbols.lookupExact("src/user.ts", "name")).toBe(
				"prop:orphan.name",
			);
			expect(
				model.fields.lookupFieldByOwner("class:User", "name"),
	...
	describe("registration dispatch table (SM-22)", () => {
		...
		it("registering an inert NodeLabel only populates the file index", () => {
			...
			expect(model.symbols.lookupExact("src/main.ts", "CONFIG")).toBe(
				"var:CONFIG",
			);
		...
		it("Method-without-ownerId skips methods.register and falls back to the callable index", () => {
			...
			expect(model.symbols.lookupExact("src/orphan.ts", "orphan")).toBe(
				"mtd:orphan",
			);
...
describe("lookupMethodByOwnerWithMRO", () => {
	...
	it("ruby-mixin: prepend provider beats class's own method (shadow)", () => {
		...
		ctx.model.symbols.add(
			"lib/prep.rb",
			"PrependedOverride",
			"trait:PrependedOverride",
			"Trait",
		);
		ctx.model.symbols.add(
			"lib/account.rb",
			"serialize",
			"method:Account:serialize",
			"Method",
			{
				returnType: "String",
	...
	it("ruby-mixin: singleton ancestryOverride routes to extend provider", () => {
		...
		const singletonAncestry = map
			.getSingletonAncestry("class:Account")
			.map((e) => e.parentId);
		const result = lookupMethodByOwnerWithMRO(
			"class:Account",
			"log",
...
describe("resolveMemberCall", () => {
	...
	it("resolves inherited method via MRO walk", () => {
		...
		ctx.importMap.set(
			"src/app.java",
			new Set(["src/child.java", "src/parent.java"]),
		);
	...
	it("resolves with heritage map across C3 MRO chain (Python)", () => {
		...
		ctx.importMap.set(
			"src/main.py",
			new Set(["src/a.py", "src/b.py", "src/c.py"]),
		);
	...
	it("homonym + shared ancestor: both walk MRO to the same method (dedups to 1)", () => {
		...
		ctx.importMap.set(
			"src/app.ts",
			new Set(["src/base.ts", "src/a.ts", "src/b.ts"]),
		);
	...
	it("C#: walks implements-split to find inherited method via interface", () => {
		...
		ctx.importMap.set(
			"src/app.cs",
			new Set(["src/iservice.cs", "src/base.cs", "src/my.cs"]),
		);
...
describe("resolveStaticCall", () => {
	...
	it("reuses the pre-computed tiered result instead of calling ctx.resolve twice", () => {
		...
		const result = resolveStaticCall(
			"User",
			"src/app.ts",
			ctx,
			undefined,
