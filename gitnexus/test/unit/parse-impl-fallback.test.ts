...
vi.mock(
	"../../src/core/ingestion/filesystem-walker.js",
	async (importOriginal) => {
		const actual =
			await importOriginal<
				typeof import("../../src/core/ingestion/filesystem-walker.js")
			>();
		return {
			...actual,
			readFileContents: vi.fn(async (repoPath: string, chunkPaths: string[]) => {
				...
				if (
					failureConfig.readFileContentsCalls >=
					failureConfig.readFileContentsFailAfter
				) {
...
vi.mock(
	"../../src/core/ingestion/call-processor.js",
	async (importOriginal) => {
		const actual =
			await importOriginal<
				typeof import("../../src/core/ingestion/call-processor.js")
			>();
		return {
			...actual,
			processCalls: vi.fn(async (...args: unknown[]) => {
				...
				return (
					actual.processCalls as unknown as (...a: unknown[]) => Promise<unknown>
				)(...args);
			}),
		};
	},
...
vi.mock("../../src/core/ingestion/ast-cache.js", async (importOriginal) => {
	const actual =
		await importOriginal<
			typeof import("../../src/core/ingestion/ast-cache.js")
		>();
	return {
		...actual,
...
const { runChunkedParseAndResolve } = await import(
