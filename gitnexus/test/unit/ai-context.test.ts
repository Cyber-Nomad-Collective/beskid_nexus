...
describe("generateAIContextFiles", () => {
	...
	it("preserves volatile counts when noStats is not set (default)", async () => {
		const subDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "gn-with-stats-test-"),
		);
		const subStorage = path.join(subDir, ".gitnexus");
		await fs.mkdir(subStorage, { recursive: true });
		try {
...
`;
	...
	it("does not create .claude/skills/gitnexus/ when skipSkills is true (#742)", async () => {
		...
		const skipDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "gn-ai-ctx-skip-skills-"),
		);
		...
		try {
			...
			expect(result.files).toContain(
				".claude/skills/gitnexus/ (skipped via --skip-skills)",
			);
			await expect(
	...
	it("writes nothing when both skipAgentsMd and skipSkills are true (--index-only, #742)", async () => {
		...
		const idxDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "gn-ai-ctx-index-only-"),
		);
		...
		try {
			...
			await expect(
				fs.access(path.join(idxDir, ".claude", "skills", "gitnexus")),
			).rejects.toThrow();
		} finally {
	...
	it("omits standard skill references from AGENTS.md/CLAUDE.md when skipSkills is true (#742)", async () => {
		...
		const noStdDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "gn-ai-ctx-no-std-skills-"),
		);
	...
	it("preserves inline marker references in prose and does not corrupt markdown (#1041)", async () => {
		...
		try {
			...
			expect(
				contentAfter,
				"inline prose line must survive the first run verbatim",
			...
			expect((contentAfter.match(/<!-- gitnexus:start -->/g) || []).length).toBe(
				2,
			);
			...
			expect(
				contentAfter,
				"inline prose line must survive the second run verbatim",
			).toContain(inlineProseLine);
			expect((contentAfter.match(/<!-- gitnexus:start -->/g) || []).length).toBe(
				2,
			);
