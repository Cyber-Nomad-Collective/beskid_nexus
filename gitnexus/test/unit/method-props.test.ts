...
describe("typeTagForId", () => {
	it("returns type tag for same-arity collision: find(int) vs find(String)", () => {
		const findInt = makeMethodInfo("find", [{ name: "id", type: "int" }], {
			line: 10,
		});
		const findString = makeMethodInfo(
			"find",
	...
	it("returns empty string for single method (no collision)", () => {
		const save = makeMethodInfo("save", [{ name: "user", type: "User" }], {
			line: 10,
		});
	...
	it("returns empty string when same name but different arity (no collision in group)", () => {
		const find1 = makeMethodInfo("find", [{ name: "id", type: "int" }], {
			line: 10,
		});
	...
	it("returns empty string when collision exists but a method has null type", () => {
		...
		const findUntyped = makeMethodInfo("find", [{ name: "name", type: null }], {
			line: 15,
		});
	...
	it("handles constructor overloads the same as methods", () => {
		const ctorInt = makeMethodInfo("constructor", [{ name: "id", type: "int" }], {
			line: 5,
		});
		const ctorString = makeMethodInfo(
			"constructor",
			[{ name: "name", type: "String" }],
			{
				line: 10,
	...
	it("returns empty string for TypeScript (overload signatures should collapse)", () => {
		const findNum = makeMethodInfo("find", [{ name: "id", type: "number" }], {
			line: 10,
		});
		const findStr = makeMethodInfo("find", [{ name: "name", type: "string" }], {
			line: 15,
		});
		...
		expect(
			typeTagForId(map, "find", 1, findNum, SupportedLanguages.TypeScript),
		).toBe("");
		expect(
			typeTagForId(map, "find", 1, findStr, SupportedLanguages.JavaScript),
		).toBe("");
	});
...
describe("constTagForId", () => {
	...
	it("works with typed parameters: find(int) vs find(int) const", () => {
		...
		const findNonConst = makeMethodInfo("find", [{ name: "id", type: "int" }], {
			line: 15,
		});
