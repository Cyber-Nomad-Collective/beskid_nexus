...
describe("understand-quickly helpers (gitnexus-shared)", () => {
	...
	describe("parseOwnerRepoFromRemote", () => {
		it.each([
			...
			[
				"https://github.com/looptech-ai/understand-quickly.git",
				"looptech-ai/understand-quickly",
			],
			[
				"ssh://git@github.com/abhigyanpatwari/GitNexus.git",
		...
		it.each([null, undefined, "", "   ", "not-a-url", "https://github.com/"])(
			...
			(input) => {
				expect(
					parseOwnerRepoFromRemote(input as string | null | undefined),
				).toBeNull();
			},
		);
	});
	...
	describe("buildUqDispatchPayload", () => {
		...
		it("throws on a malformed id rather than building an invalid payload", () => {
			...
			expect(() => buildUqDispatchPayload("/Users/me/repo")).toThrow(
				/owner\/repo/,
			);
		});
	});
...
describe("publishCommand (no-token no-op)", () => {
	...
	it("exits 0 without firing a network call when the token is unset", async () => {
		const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(() => {
			throw new Error(
				"publishCommand should NOT call fetch when the token is missing",
			);
		});
	...
	it("exits 0 with no token even when no index/repo exists (BLOCKER 2)", async () => {
		...
		const noIndexDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "gn-publish-noidx-"),
		);
		try {
			const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(() => {
				throw new Error(
