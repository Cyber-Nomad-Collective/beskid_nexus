...
import { isCobolFile, isJclFile, processCobol } from "../cobol-processor.js";
...
export const cobolPhase: PipelinePhase<CobolOutput> = {
	...
	async execute(
		...
		const cobolScanned = scannedFiles.filter(
			(f) => isCobolFile(f.path) || isJclFile(f.path),
		);
		...
		if (isDev) {
			...
			if (cobolResult.jclJobs > 0) {
				logger.info(
					`  JCL: ${cobolResult.jclJobs} jobs, ${cobolResult.jclSteps} steps`,
				);
			}
		}
