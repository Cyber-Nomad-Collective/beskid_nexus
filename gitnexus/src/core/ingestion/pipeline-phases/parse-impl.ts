...
import { type ASTCache, createASTCache } from "../ast-cache.js";
import {
	BindingAccumulator,
	type BindingEntry,
...
import { mergeChunkResults, processParsing } from "../parsing-processor.js";
		...
		for (let chunkIdx = 0; chunkIdx < numChunks; chunkIdx++) {
			...
			if (cachedRaw && cachedRaw.length > 0) {
				...
				onProgress({
					phase: "parsing",
					percent: Math.round(
						20 + ((filesParsedSoFar + cachedFiles) / totalParseable) * 62,
					),
