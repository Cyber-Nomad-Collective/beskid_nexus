...
export async function runCrossFileBindingPropagation(
	...
	for (const level of levels) {
		...
		for (const {
			...
			if (
				crossFileResolved % PROGRESS_INTERVAL === 0 ||
				crossFileResolved === cappedTotal
			) {
				const pct =
					cappedTotal > 0 ? Math.round((crossFileResolved / cappedTotal) * 8) : 0;
				onProgress({
					phase: "parsing",
					percent: 82 + pct,
					message: `Cross-file type propagation (${crossFileResolved}/${cappedTotal} files)...`,
					stats: {
						filesProcessed: crossFileResolved,
						totalFiles,
						nodesCreated: graph.nodeCount,
					},
				});
