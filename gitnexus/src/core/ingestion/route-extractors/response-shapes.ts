...
function buildShapeResult(
	...
	return {
		...(successKeys.length > 0
			? { responseKeys: [...new Set(successKeys)] }
			: {}),
...
export function detectStatusCode(
	...
	if (closingBracePos > 0) {
		const afterFirstArg = content.slice(
			closingBracePos + 1,
			closingBracePos + 150,
		);
		const secondArgMatch = afterFirstArg.match(
			/^\s*,\s*\{[^}]*status\s*:\s*(\d{3})/,
		);
		if (secondArgMatch) {
	...
	const extendedBefore = content.slice(
		Math.max(0, jsonMatchPos - 300),
		jsonMatchPos,
	);
	if (
		/new\s+Response\s*\(\s*JSON\s*\.stringify\s*$/.test(extendedBefore) &&
		closingBracePos > 0
	) {
		const afterStringify = content.slice(
			closingBracePos + 1,
			closingBracePos + 200,
		);
		const respStatusMatch = afterStringify.match(
			/^\s*\)\s*,\s*\{[^}]*status\s*:\s*(\d{3})/,
		);
		if (respStatusMatch) {
...
function detectPHPStatusCode(
	content: string,
	jsonEncodePos: number,
): number | undefined {
