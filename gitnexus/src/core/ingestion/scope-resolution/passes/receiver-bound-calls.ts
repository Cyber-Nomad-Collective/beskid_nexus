...
function resolveClassBindingForName(
	...
	const wantedArgs = extractTemplateArguments(rawClassName)?.map(
		normalizeTemplateArgToken,
	);
		...
		for (const site of parsed.referenceSites) {
			...
			if (receiverName.includes(".") || receiverName.includes("(")) {
				...
				if (currentClass !== undefined) {
					const chain = [
						currentClass.nodeId,
						...scopes.methodDispatch.mroFor(currentClass.nodeId),
					];
					let memberDef: SymbolDefinition | undefined;
			...
			if (
				provider.resolveThisViaEnclosingClass === true &&
				receiverName === "this"
			) {
				const enclosingClass = findEnclosingClassDef(site.inScope, scopes);
				if (enclosingClass !== undefined) {
					...
					for (const ownerId of chain) {
						const methodOverloads = model.methods.lookupAllByOwner(
							ownerId,
							memberName,
						);
						...
						const reason =
							site.kind === "write" || site.kind === "read"
								...
								: memberDef.filePath !== parsed.filePath
									...
									: "global";
						const confidence =
							site.kind === "write" || site.kind === "read" ? 1.0 : 0.85;
						const ok = tryEmitEdge(
							graph,
							scopes,
							nodeLookup,
			...
			if (classDef !== undefined) {
				const chain = [
					classDef.nodeId,
					...scopes.methodDispatch.mroFor(classDef.nodeId),
				];
				let memberDef: SymbolDefinition | undefined;
				for (const ownerId of chain) {
					memberDef = findOwnedMember(ownerId, memberName, model);
					if (memberDef !== undefined) {
						...
						if (
							narrowOverloadCandidates([memberDef], site.arity, site.argumentTypes)
								.length === 0
						) {
							memberDef = undefined;
							break;
			...
			if (
				typeRef !== undefined &&
				chainHead !== undefined &&
				!namespaceTargets.has(chainHead)
			) {
				...
				if (ownerDef !== undefined) {
					const chain = [
						ownerDef.nodeId,
						...scopes.methodDispatch.mroFor(ownerDef.nodeId),
					];
					let memberDef: SymbolDefinition | undefined;
					for (const ownerId of chain) {
						memberDef = findOwnedMember(ownerId, memberName, model);
