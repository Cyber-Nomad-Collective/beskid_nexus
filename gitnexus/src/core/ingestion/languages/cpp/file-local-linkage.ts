...
export function markCppAnonymousNamespaceRange(
	filePath: string,
	range: RangeKeyShape,
): void {
...
export function populateCppNonGloballyVisible(parsed: {
	...
	for (const scope of parsed.scopes) {
		...
		if (scope.kind === "Namespace" && isCppInlineNamespaceScope(scope.id))
			continue;
		...
		if (scope.kind === "Namespace" && anonymousNamespaceScopeIds.has(scope.id))
			continue;
 ...
 */
export function isCppDefGloballyVisible(
	filePath: string,
	nodeId: string,
): boolean {
