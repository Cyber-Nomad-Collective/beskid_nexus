 ...
 */
export function splitNamespaceUseDeclaration(
	stmtNode: SyntaxNode,
): CaptureMatch[] {
...
function parseSingleUseClause(
	...
	const qualName =
		findNamedChild(node, "qualified_name") ?? findNamedChild(node, "name");
...
function parseUseClause(
	...
	const qualName =
		findNamedChild(clause, "qualified_name") ?? findNamedChild(clause, "name");
	if (qualName === null) return null;
	const source = qualName.text.trim();
	if (source === "") return null;
	...
	if (aliasClause !== null) {
		...
		const aliasName =
			findNamedChild(aliasClause, "name") ?? aliasClause.firstNamedChild;
...
function decomposeGrouped(
	...
	const prefixNode =
		findNamedChild(stmtNode, "qualified_name") ??
		findNamedChild(stmtNode, "name");
