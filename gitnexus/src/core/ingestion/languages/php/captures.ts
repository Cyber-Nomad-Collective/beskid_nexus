...
type SyntaxNode = ReturnType<
	ReturnType<typeof getPhpParser>["parse"]
>["rootNode"];
...
export function emitPhpScopeCaptures(
	...
	let tree = cachedTree as
		| ReturnType<ReturnType<typeof getPhpParser>["parse"]>
		| undefined;
	if (tree === undefined) {
...
function normalizePhpDocType(raw: string): string | null {
	...
	if (type.includes("|")) {
		const parts = type
			...
			.filter(
				(p) =>
					p !== "null" && p !== "false" && p !== "void" && p !== "mixed" && p !== "",
			);
...
function synthesizePhpDocBindings(fnNode: SyntaxNode): CaptureMatch[] {
	...
	while ((m = PHPDOC_PARAM_RE.exec(commentBlock)) !== null) {
		...
		out.push({
			"@type-binding.parameter": nodeToCapture(
