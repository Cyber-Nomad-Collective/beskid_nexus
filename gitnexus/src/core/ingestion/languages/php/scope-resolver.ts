...
import {
	getPhpNamespaceForFile,
	populatePhpNamespaceSiblings,
...
function phpEmitUnresolvedReceiverEdges(
	...
	for (const parsed of parsedFiles) {
		for (const site of parsed.referenceSites) {
			...
			const typeRef = findReceiverTypeBinding(
				site.inScope,
			...
			if (
				narrowOverloadCandidates([fnDef], site.arity, site.argumentTypes).length ===
				0
			) {
				continue;
			}
...
const phpScopeResolver: ScopeResolver = {
	...
	buildMro: (graph, parsedFiles, nodeLookup) =>
		buildPhpMro(graph, parsedFiles, nodeLookup),
