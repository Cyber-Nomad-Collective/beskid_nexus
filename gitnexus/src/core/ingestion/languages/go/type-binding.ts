...
import { type SyntaxNode, syntheticCapture } from "../../utils/ast-helpers.js";
			...
			if (expr?.type === "call_expression") {
				...
				if (fn?.type === "identifier" && fn.text === "new" && args !== null) {
					...
					if (typeArg !== undefined) {
						...
						const nameNodes = lhs.namedChildren.filter(
							(c) => c.type === "identifier",
						);
					...
					if (sliceOrMap !== undefined) {
						...
						if (sliceOrMap.type === "slice_type") {
							...
							if (valueType !== undefined)
								typeName = extractSimpleTypeNameText(valueType);
						}
						if (typeName !== "") {
							const nameNodes = lhs.namedChildren.filter(
								(c) => c.type === "identifier",
							...
							if (nameNodes.length > 0) {
								out.push({
									"@type-binding.make": syntheticCapture(
										"@type-binding.make",
										node,
										"make",
									),
...
function synthesizeElementAccessBindings(
	rootNode: SyntaxNode,
	out: CaptureMatch[],
): void {
	...
	for (const node of rootNode.descendantsOfType("short_var_declaration")) {
		...
		for (const expr of right.namedChildren) {
			...
			if (expr.type === "call_expression") {
				...
				if (
					fn?.type === "identifier" &&
					(fn.text === "make" || fn.text === "new")
				) {
					const args = expr.childForFieldName("arguments");
					const typeNode = args?.namedChildren.find((c) =>
						["type_identifier", "qualified_type", "slice_type", "map_type"].includes(
							c.type,
						),
					);
					if (typeNode) {
	...
	for (const rangeClause of rootNode.descendantsOfType("range_clause")) {
		...
		if (
			rangeExpr.type === "identifier" ||
			rangeExpr.type === "selector_expression"
		) {
		...
		for (const child of left.namedChildren) {
			if (child.type === "identifier") {
				...
				out.push({
					"@type-binding.range": syntheticCapture(
						"@type-binding.range",
						rangeClause,
						child.text,
					),
	...
	for (const node of rootNode.descendantsOfType("call_expression")) {
		...
		if (operand.type === "index_expression") {
			...
			if (base?.type === "identifier" && varElementType.has(base.text)) {
				...
				out.push({
					"@type-binding.index": syntheticCapture(
						"@type-binding.index",
						node,
						operand.text,
					),
