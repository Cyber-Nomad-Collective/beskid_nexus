...
export function emitCsharpScopeCaptures(
	...
	for (const m of rawMatches) {
		...
		if (grouped["@reference.read.member"] !== undefined) {
			...
			const memberNode = findNodeAtRange(
				tree.rootNode,
				anchor.range,
...
function synthesizeGenericTypeArgumentReferences(
	root: SyntaxNode,
): CaptureMatch[] {
	const out: CaptureMatch[] = [];
...
function terminalTypeNameNode(node: SyntaxNode): SyntaxNode | null {
	switch (node.type) {
		case "identifier":
			return node;
		case "nullable_type":
			return node.firstNamedChild === null
				? null
				: terminalTypeNameNode(node.firstNamedChild);
		case "qualified_name":
