...
export function interpretCsharpImport(
	...
	switch (kind) {
		case "namespace": {
			...
			return {
				kind: "namespace",
				localName:
					nameCap?.text ?? sourceCap.text.split(".").pop() ?? sourceCap.text,
...
export function interpretCsharpTypeBinding(
	captures: CaptureMatch,
	...
	const rawType = stripQualifier(
		stripGeneric(stripNullable(typeCap.text.trim())),
	);
