...
export function arityForIdFromInfo(info: MethodInfo): number | undefined {
	return info.parameters.some((p) => p.isVariadic)
		? undefined
...
export function typeTagForId(
	...
	for (const info of sameArityGroup) {
		if (
			...
			info.parameters.some((p) => (p.rawType ?? p.type) === null)
		) {
			return "";
		}
	}
	...
	const types = currentInfo.parameters.map(
		(p) => (p.rawType ?? p.type) as string,
	);
	return `~${types.join(",")}`;
}
