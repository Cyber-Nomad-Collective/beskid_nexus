...
function swiftNavigationSuffixName(
	node: SyntaxNode | null,
): string | undefined {
	return node?.type === "navigation_suffix"
...
const extractInitializer: InitializerExtractor = (
	...
	if (!callExpr) {
		...
		for (let i = 0; i < node.namedChildCount; i++) {
			...
			if (
				child &&
				(child.type === "await_expression" || child.type === "try_expression")
			) {
...
const scanConstructorBinding: ConstructorBindingScanner = (node) => {
	...
	if (callee.type === "navigation_expression") {
		...
		if (
			receiver?.type === "simple_identifier" &&
			swiftNavigationSuffixName(suffix) === "init"
		) {
	...
	for (let i = 0; i < node.namedChildCount; i++) {
		...
		if (child.type === "pattern" || child.type === "simple_identifier") {
			if (!loopVarName) {
				...
				const varName =
					extractVarName(child) ??
					(child.type === "simple_identifier" ? child.text : undefined);
 ...
 */
function extractSwiftElementTypeFromTypeNode(
	typeNode: SyntaxNode,
): string | undefined {
