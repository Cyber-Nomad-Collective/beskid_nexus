...
import type { ExtractedORMQuery, ExtractedRoute } from "./protocol.js";
...
export const HTTP_CLIENT_ONLY_METHODS = new Set([
	"head",
	"options",
	"request",
	"ajax",
...
function getCallMethodName(node: SyntaxNode): string | null {
	const nameNode =
		...
		node.children?.find((c: SyntaxNode) => c.type === "name");
...
function extractClassArg(argsNode: SyntaxNode | null): string | null {
	...
	for (const child of argsNode.children ?? []) {
		...
		if (target?.type === "class_constant_access_expression") {
			return (
				target.children?.find((c: SyntaxNode) => c.type === "name")?.text ?? null
			);
		}
	}
	return null;
...
function extractControllerTarget(argsNode: SyntaxNode | null): {
	...
	if (handlerNode.type === "array_creation_expression") {
		...
		if (elements[0]) {
			...
			if (classAccess) {
				controller =
					classAccess.children?.find((c: SyntaxNode) => c.type === "name")?.text ??
					null;
			}
		}
		if (elements[1]) {
	...
	if (handlerNode.type === "class_constant_access_expression") {
		const controller =
			handlerNode.children?.find((c: SyntaxNode) => c.type === "name")?.text ??
			null;
			...
			for (const el of target.children ?? []) {
				...
				if (key === "middleware") {
					...
					if (val?.type === "class_constant_access_expression") {
						ctx.controller =
							val.children?.find((c: SyntaxNode) => c.type === "name")?.text ?? null;
					}
				}
			}
		}
