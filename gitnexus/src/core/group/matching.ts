import type { CrossLink, MatchingConfig, StoredContract } from "./types.js";
...
function isServiceWildcard(cid: string): boolean {
	return (
		(cid.startsWith("grpc::") || cid.startsWith("thrift::")) && cid.endsWith("/*")
	);
}
...
function buildNoisyContractFilter(
	...
	const excludePaths = matchingConfig?.exclude_links_paths?.length
		? new Set(
				matchingConfig.exclude_links_paths.map((p) => p.replace(/\/+$/, "")),
			)
		: new Set<string>();
	const excludeParamOnly =
		matchingConfig?.exclude_links_param_only_paths === true;
...
export function buildProviderIndex(
	...
	const providers = contracts.filter(
		(c) => c.role === "provider" && !isNoisy(c.contractId),
	);
...
export function runExactMatch(
	...
	const consumers = contracts.filter(
		(c) =>
			c.role === "consumer" &&
			!isServiceWildcard(c.contractId) &&
			!isNoisy(c.contractId),
	);
	...
	const normalUnmatched = contracts.filter((c) => {
		...
		return c.role === "provider"
			? !matchedProviderIds.has(id)
			: !matchedConsumerIds.has(id);
	});

	// Re-add service wildcard contracts — they were never in exact matching
	const serviceWildcards = contracts.filter((c) =>
		isServiceWildcard(c.contractId),
	);
...
export function runWildcardMatch(
	...
	for (const consumer of wildcardConsumers) {
		...
		if (
			consumerType === "thrift" &&
			!fqService.includes(".") &&
			matchedProviderServices.size > 1
		) {
			continue;
		}
