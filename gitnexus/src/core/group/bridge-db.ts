...
async function removeLbugFile(basePath: string): Promise<void> {
	const candidates = [
		basePath,
		...LBUG_SIDECAR_SUFFIXES.map((s) => `${basePath}${s}`),
	];
...
export function indexContract(
	...
	if (contract.symbolUid) {
		index.byUid.set(
			uidKey(contract.repo, contract.role, contract.symbolUid),
			nodeId,
		);
	}
	index.byRef.set(
		refKey(
			contract.repo,
			contract.role,
			contract.symbolRef.filePath,
			contract.symbolRef.name,
		),
		nodeId,
	);
...
export async function writeBridgeMeta(
	groupDir: string,
	meta: BridgeMeta,
): Promise<void> {
...
export async function openBridgeDbReadOnly(
	groupDir: string,
	...
	for (let attempt = 1; attempt <= LBUG_OPEN_RETRY_ATTEMPTS; attempt++) {
		...
		try {
			...
			const delay = Math.min(
				LBUG_OPEN_RETRY_BASE_MS * attempt,
				LBUG_OPEN_RETRY_MAX_MS,
			);
