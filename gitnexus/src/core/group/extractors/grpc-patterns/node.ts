 ...
 */
function resolveGrpcClientFieldType(
	...
	if (
		...
		parent.type.endsWith("field_definition")
	) {
...
function scanBundle(
	...
	const usesLoadPackage =
		runCompiledPatterns(bundle.loadPackageDefinition, tree).length > 0;
	if (usesLoadPackage) {
