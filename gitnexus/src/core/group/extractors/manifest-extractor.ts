 ...
 */
function parseHttpContract(raw: string): {
	method: string | null;
	path: string;
} {
...
export class ManifestExtractor {
	async extractFromManifest(
		...
		for (const {
			...
			const providerUid =
				providerSymbol?.uid || manifestSymbolUid(providerRepo, contractId);
			const consumerUid =
				consumerSymbol?.uid || manifestSymbolUid(consumerRepo, contractId);
