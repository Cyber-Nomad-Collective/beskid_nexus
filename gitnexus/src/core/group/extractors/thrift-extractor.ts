...
export function thriftServiceContractId(
	namespace: string,
	serviceName: string,
): string {
	const prefix = namespace ? `${namespace}.${serviceName}` : serviceName;
...
function extractNamespace(sanitizedContent: string): string {
	...
	const namespaceRe =
		/^\s*namespace\s+([A-Za-z_*][\w.*-]*)\s+([A-Za-z_][\w.]*)\s*$/gm;
...
function extractServiceBlocks(
	sanitizedContent: string,
): Array<{ name: string; body: string }> {
	const results: Array<{ name: string; body: string }> = [];
	const headerRe =
		/service\s+([A-Za-z_]\w*)\s*(?:extends\s+[A-Za-z_][\w.]*)?\s*\{/g;
...
export async function buildThriftContext(
	repoPath: string,
): Promise<ThriftContext> {
