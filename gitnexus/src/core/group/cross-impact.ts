...
export function clampTimeout(timeoutMs: number): number {
	...
	return Math.min(
		...
		Math.max(IMPACT_TIMEOUT_MIN_MS, Math.trunc(timeoutMs)),
	);
}
...
export function validateGroupImpactParams(params: Record<string, unknown>):
	...
	const { depth: crossDepth, warning: crossDepthWarning } = clampCrossDepth(
		params.crossDepth,
	);
	...
	let minConfidence =
		typeof params.minConfidence === "number" ? params.minConfidence : 0;
...
export function collectImpactSymbolUids(
	...
	if (target?.id) {
		targetFilePath =
			typeof target.filePath === "string" ? target.filePath : undefined;
...
function rowToNeighbor(r: Record<string, unknown>): BridgeNeighborRow | null {
	...
	return {
		...
		neighborUid,
		neighborFilePath:
			r.neighborFilePath !== undefined
				? String(r.neighborFilePath)
				: String(r[2] ?? ""),
...
export async function runGroupImpact(
	...
	if (uids.length === 0) {
		...
		return {
			...
			timeoutMs,
			truncationReason: (local as { partial?: boolean }).partial
				? "partial"
				: undefined,
			crossDepthWarning,
		};
	...
	try {
		const cypher =
			direction === "upstream" ? CY_NEIGHBORS_UPSTREAM : CY_NEIGHBORS_DOWNSTREAM;
		...
		for (const n of neighbors) {
			...
			cross.push({
				...
				by_depth: ((fan as { byDepth?: unknown }).byDepth ?? {}) as Record<
					string,
					unknown[]
				>,
	...
	const localSum =
		(local as { summary?: Record<string, number> })?.summary || {};
