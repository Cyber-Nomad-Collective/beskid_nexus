...
export function parseGroupConfig(yamlContent: string): GroupConfig {
	const raw = yaml.load(yamlContent, { schema: yaml.JSON_SCHEMA }) as Record<
		string,
		unknown
	>;
	...
	const links: GroupManifestLink[] = rawLinks.map((l: unknown, i: number) => {
		...
		if (!VALID_ROLES.includes(link.role as ContractRole)) {
			throw new Error(
				`links[${i}].role "${link.role}" is invalid. Expected: provider | consumer`,
			);
		}
		if (
	...
	const packages =
		(raw.packages as Record<string, Record<string, string>>) || {};
