...
} from "./service-boundary-detector.js";
...
export function stableRepoPoolId(
	entry: RegistryEntry,
			...
			for (const [groupPath, regName] of Object.entries(config.repos)) {
				...
				try {
					...
					if (config.detect.thrift) {
						const extracted = await thriftEx.extract(
							executor,
							handle.repoPath,
							handle,
						);
					...
					if (config.detect.topics) {
						const extracted = await topicEx.extract(
							executor,
							handle.repoPath,
							handle,
						);
					...
					if (config.detect.includes) {
						const extracted = await includeEx.extract(
							executor,
							handle.repoPath,
							handle,
						);
