...
function filterQueryByServicePrefix(
	...
	const allowed = new Set(
		symbols
			.map((s) => String((s as { process_id?: string }).process_id ?? ""))
			.filter(Boolean),
	);
	const processes = (queryResult.processes || []).filter((p) =>
		allowed.has(String(p.id)),
	);
...
> {
	...
	try {
		...
		if ((e as NodeJS.ErrnoException).code === "ENOENT") {
			return {
				ok: false,
				error: `No contracts.json for this group. Run group_sync first.`,
			};
		}
	...
	const registry: ContractRegistry = {
		...
		repoSnapshots:
			base.repoSnapshots &&
			typeof base.repoSnapshots === "object" &&
			base.repoSnapshots !== null
				? (base.repoSnapshots as Record<
						string,
						{ indexedAt: string; lastCommit: string }
					>)
				: {},
...
export class GroupService {
	...
	async groupContracts(params: Record<string, unknown>): Promise<unknown> {
		...
		if (loaded.ok === false) {
			if (loaded.error.includes("No contracts.json")) {
				return {
					error: `No contracts.json for group "${name}". Run group_sync first.`,
				};
			}
		...
		if (params.unmatchedOnly) {
			...
			contracts = contracts.filter(
				(c) => !matchedIds.has(`${c.repo}::${c.contractId}`),
			);
		}
	...
	async groupContext(
		...
		if (
			...
			return {
				group: name || "",
				error: "service must not be an empty string",
				results: [],
			};
		}
		const servicePrefix = normalizeServicePrefix(params.service);
		const subgroup =
			typeof params.subgroup === "string" ? params.subgroup : undefined;
			...
			memberEntries.map(async ([repoPath, registryName]) => {
				try {
					...
					if (servicePrefix) {
						...
						if (
							st === "found" &&
							!fileMatchesServicePrefix(sym?.filePath, servicePrefix)
						) {
	...
	async groupQuery(params: Record<string, unknown>): Promise<unknown> {
		...
		const allProcesses = perRepo.flatMap(
			(r) => r.processes as Array<Record<string, unknown>>,
		);
		allProcesses.sort(
			(a, b) => (b._rrf_score as number) - (a._rrf_score as number),
		);
		...
		for (const [repoPath, registryName] of Object.entries(config.repos)) {
			try {
				...
				const contractsStale =
					snapshot && meta.indexedAt
						? snapshot.indexedAt !== meta.indexedAt
						: !snapshot;
