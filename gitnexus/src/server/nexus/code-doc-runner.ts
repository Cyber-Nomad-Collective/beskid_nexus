...
export const loadDocEntities = async (
    ...
    `);
		...
		for (const row of nodes) {
			...
			const startLine =
				row.startLine !== undefined ? Number(row.startLine) : undefined;
...
const buildEntityContext = (entity: CodeDocEntity): string =>
	[
		...
		entity.startLine !== undefined
			? `Lines: ${entity.startLine}-${entity.endLine ?? entity.startLine}`
			: null,
		entity.neighborIds.length
			? `Neighbors: ${entity.neighborIds.join(", ")}`
			: null,
...
const generateCodeDocText = async (
	entity: CodeDocEntity,
): Promise<string | null> => {
	...
	const writeCall = result.toolCalls.find(
		(call) => call.name === "write_code_doc",
	);
	const codeDoc =
		typeof writeCall?.arguments.codeDoc === "string"
			? writeCall.arguments.codeDoc.trim()
			: "";
	if (codeDoc) return codeDoc;
	return result.message.content?.trim() || null;
};

const generateSpecLinks = async (
	entity: CodeDocEntity,
	...
	const resolveCall = first.toolCalls.find(
...
export const runCodeDocPipeline = async (
	params: RunCodeDocPipelineParams,
): Promise<void> => {
	...
	if (!(await isOpenRouterConfigured())) {
		...
		logger.warn(
			{ registryName },
			"Code doc pipeline skipped: OPENROUTER_API_KEY not set",
		);
		return;
	}
	...
	try {
		...
		for (const entity of entities) {
			...
			const codeDoc = await generateCodeDocText(entity);
			if (!codeDoc) continue;

			if (containsSpecProse(codeDoc, excerpts)) {
				logger.warn(
					{ entityId: entity.entityId },
					"Rejected codeDoc containing spec prose",
				);
				continue;
			}
...
export const startCodeDocJob = async (
	...
	if (existing) {
		return {
			...
			status: "skipped",
			reason: "code-doc job already running",
		};
	}
	...
	child.on("message", (msg: any) => {
		if (msg?.type === "error") {
			logger.warn(
				{ registryName: params.registryName, err: msg.message },
				"Code doc worker failed",
			);
		}
	});
