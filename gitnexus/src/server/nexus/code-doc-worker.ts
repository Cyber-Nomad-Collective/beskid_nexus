...
process.on("uncaughtException", (err) => {
	send({
		type: "error",
		message: err?.message || "Uncaught exception in code-doc worker",
	});
...
process.on("unhandledRejection", (reason: any) => {
	send({
		type: "error",
		message: reason?.message || "Unhandled rejection in code-doc worker",
	});
	setTimeout(() => process.exit(1), 500);
});
...
process.on("message", async (msg: StartMessage) => {
	...
	try {
		send({
			type: "progress",
			phase: "generating",
			message: "Generating code documentation...",
		});
