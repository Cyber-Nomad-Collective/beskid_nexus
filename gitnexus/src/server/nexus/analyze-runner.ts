	...
	if (params.repoUrl && params.skipIfCommit) {
		...
		if (remote && remote === params.skipIfCommit) {
			return {
				...
				status: "skipped",
				skipped: true,
				reason: "already indexed at HEAD",
			};
		}
	}
			...
			if (params.repoUrl && !params.repoPath) {
				...
				jobManager.updateJob(job.id, {
					...
					progress: {
						phase: "cloning",
						percent: 0,
						message: `Cloning ${params.repoUrl}...`,
					},
				});
			...
			const forkWorker = () => {
								...
								if (catalogEntryId && targetPath) {
									try {
										...
										void startCodeDocJob({
											...
											logger.warn(
												{ err: docErr, catalogEntryId },
												"Failed to start code-doc job",
											);
										});
									} catch (err) {
										logger.warn(
											{ err, catalogEntryId },
											"Failed to update catalog after analyze",
										);
									}
								}
							})
				...
				child.on("exit", (code) => {
					...
					if (j.retryCount < MAX_WORKER_RETRIES) {
						j.retryCount++;
						const delay = 1000 * 2 ** (j.retryCount - 1);
						jobManager.updateJob(job.id, {
							status: "analyzing",
							progress: {
								phase: "retrying",
