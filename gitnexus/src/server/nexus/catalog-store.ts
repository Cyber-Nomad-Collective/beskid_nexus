...
export const getCatalogEntry = async (
	id: string,
): Promise<NexusCatalogEntry | null> => {
	const catalog = await readCatalog();
...
export const createCatalogEntry = async (
	input: Omit<
		NexusCatalogEntry,
		"id" | "createdAt" | "updatedAt" | "enabled" | "sortOrder"
	> & {
		id?: string;
		enabled?: boolean;
		sortOrder?: number;
	...
	return withCatalogLock(async () => {
		const catalog = await readCatalog();
		const id = input.id
			? slugifyCatalogId(input.id)
			: slugifyCatalogId(input.displayName);
		if (catalog.entries.some((e) => e.id === id)) {
			throw new Error(`Catalog entry "${id}" already exists`);
		}
		const now = new Date().toISOString();
		const maxOrder = catalog.entries.reduce(
...
export const listPublicCatalog = async (): Promise<PublicCatalogEntry[]> => {
	...
	return Promise.all(
		publicEntries.map(async (entry) => ({
			...entry,
			docStatus: entry.registryName
				? await readCodeDocStatus(entry.registryName)
				: "idle",
		})),
	);
};
...
export const findCatalogByGitUrl = async (
	url: string,
): Promise<NexusCatalogEntry | null> => {
	const needle = normalizeGitRepoUrl(url);
	const entries = await listCatalogEntries();
	return entries.find((e) => normalizeGitRepoUrl(e.gitUrl) === needle) ?? null;
};
