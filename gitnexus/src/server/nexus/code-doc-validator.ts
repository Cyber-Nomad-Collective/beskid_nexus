...
export const containsSpecProse = (
	codeDoc: string,
	excerpts: string[],
): boolean => {
...
export const validateSpecLinks = <T extends { title: string; href: string }>(
	links: T[],
	index: Set<string>,
): T[] => links.filter((link) => index.has(link.href)).slice(0, MAX_SPEC_LINKS);
...
export const dedupeSimilarCodeDocs = <
	T extends { entityId: string; codeDoc: string },
>(
	records: T[],
	threshold = 0.85,
): T[] => {
	...
	for (const record of records) {
		const duplicate = kept.find(
			(existing) =>
				jaccardSimilarity(existing.codeDoc, record.codeDoc) > threshold,
		);
