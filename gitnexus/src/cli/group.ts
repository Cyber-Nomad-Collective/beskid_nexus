...
export function registerGroupCommands(program: Command): void {
	...
	group
		...
		.action(async (name: string, opts: { force?: boolean }) => {
			const { createGroupDir, getDefaultGitnexusDir } = await import(
				"../core/group/storage.js"
			);
	...
	group
		...
		.action(
			async (groupName: string, groupPath: string, registryName: string) => {
				const { getGroupDir, getDefaultGitnexusDir } = await import(
					"../core/group/storage.js"
				);
				...
				console.log(
					`Added ${registryName} as "${groupPath}" to group "${groupName}"`,
				);
	...
	group
		...
		.action(async (name?: string) => {
			const { listGroups, getDefaultGitnexusDir, getGroupDir } = await import(
				"../core/group/storage.js"
			);
			if (!name) {
				...
				if (groups.length === 0) {
					console.log(
						"No groups configured. Create one with: gitnexus group create <name>",
					);
					return;
				}
				console.log("Groups:");
			...
			if (config.links.length > 0) {
				...
				for (const link of config.links) {
					console.log(
						`  ${link.from} -> ${link.to} [${link.type}: ${link.contract}]`,
					);
				}
			}
		});
		...
		.action(async (name: string) => {
			...
			try {
				...
				for (const [repoPath, row] of Object.entries(st.repos || {})) {
					if (row.missing) {
						console.log(
							`  ${repoPath.padEnd(25)} MISSING   (not in registry or unreadable)`,
						);
						continue;
					}
	...
	group
		...
		.action(async (name: string, opts: Record<string, boolean | undefined>) => {
			...
			console.log(
				`Syncing group "${name}" (${Object.keys(config.repos).length} repos)...\n`,
			);
			...
			if (opts.json) {
				...
				console.log(
					`  exact:     ${exactLinks.length} cross-links (confidence 1.0)`,
				);
	...
	group
		...
		.option(
			"--service <path>",
			"Optional monorepo service directory prefix (path filter)",
		)
		.option(
			"--subgroup <path>",
		...
		.option("--json", "JSON output")
		.action(
			async (name: string, opts: Record<string, string | boolean | undefined>) => {
				const { LocalBackend } = await import("../mcp/local/local-backend.js");
				...
				try {
					...
					if (opts.json) {
						...
						console.log(
							`Group impact for "${name}" (${String(opts.repo)}): risk=${risk ?? "?"}`,
						);
						if (summary) {
							console.log(
				...
				try {
					...
					if (opts.json) {
						...
						for (const p of merged.results) {
							const label = (p.summary ||
								p.heuristicLabel ||
								p.name ||
								"unnamed") as string;
							console.log(
								`  [${p._repo}] ${label} (rrf: ${(p._rrf_score as number).toFixed(4)})`,
							);
						}
				...
				try {
					...
					if (opts.json) {
						...
						for (const c of contracts) {
							console.log(
								`  [${c.role}] ${c.contractId}  (${c.repo})  ${c.symbolRef.name}`,
							);
						}
