 ...
 */
function detectIndentation(raw: string): {
	tabSize: number;
...
async function setupCursor(result: SetupResult): Promise<void> {
	...
	try {
		...
		if (ok) {
			...
			result.errors.push(
				"Cursor: mcp.json is corrupt — skipping to preserve existing content",
			);
		}
	} catch (err: any) {
...
function hasGitnexusHook(hooksObj: any, eventName: string): boolean {
	...
	return entries.some(
		(h: any) =>
			...
			h.hooks.some(
				(hh: any) =>
					typeof hh.command === "string" && hh.command.includes("gitnexus-hook"),
			),
	);
}
...
async function mergeHooksJsonc(
	...
	for (const { eventName, value } of entries) {
		...
		if (
			eventNode?.children?.[1] &&
			Array.isArray(eventNode.children[1].children)
		) {
