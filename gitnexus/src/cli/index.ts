...
program
	...
	.option(
		"--embedding-sub-batch-size <n>",
		"Number of chunks per embedding model call",
	)
	.option(
		"--embedding-device <device>",
		"Embedding device: auto, cpu, dml, cuda, or wasm",
	)
	.addHelpText(
		"after",
...
program
	...
	.option(
		"-f, --force",
		"Register even if meta.json is missing (stats will be empty)",
	)
	.option(
		"--allow-non-git",
		"Allow registering folders that are not Git repositories",
	)
...
program
	...
	.option(
		"--provider <provider>",
		"LLM provider: openai or cursor (default: openai)",
	)
	.option(
		"--model <model>",
		"LLM model or Azure deployment name (default: minimax/minimax-m2.5)",
	)
	.option(
		"--base-url <url>",
	...
	.option(
		"--api-key <key>",
		"LLM API key or Azure api-key (saved to ~/.gitnexus/config.json)",
	)
	.option(
		"--api-version <version>",
	...
	.option(
		"--no-reasoning-model",
		"Disable reasoning model mode (overrides saved config)",
	)
	.option("--concurrency <n>", "Parallel LLM calls (default: 3)", "3")
	.option(
		"--timeout <seconds>",
		"Per-attempt LLM request timeout in seconds (default: 60)",
	)
	.option("--retries <n>", "Max LLM retry attempts per request (default: 3)")
	.option("--gist", "Publish wiki as a public GitHub Gist after generation")
	.option(
		"-v, --verbose",
		"Enable verbose output (show LLM commands and responses)",
	)
	.option(
		"--review",
...
program
	...
	.option(
		"--id <owner/repo>",
		"Override the registry id (defaults to the origin remote)",
	)
	.option(
		"--skip-git",
...
program
	...
	.option(
		"-s, --scope <scope>",
		"What to analyze: unstaged, staged, all, or compare",
		"unstaged",
	)
