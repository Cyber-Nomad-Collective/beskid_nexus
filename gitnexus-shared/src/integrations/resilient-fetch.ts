...
export interface ResilientFetchOptions {
	...
	retry?: Partial<
		Pick<RetryOptions, "maxAttempts" | "baseDelayMs" | "capDelayMs">
	> & {
...
export async function resilientFetch(
	...
	const breaker =
		opts.breaker ??
		getBreaker(opts.breakerKey ?? defaultBreakerKey(input), opts.breakerOptions);
	...
	for (let attempt = 0; attempt < retryConfig.maxAttempts; attempt++) {
		...
		switch (outcome.kind) {
			...
			default: {
				...
				throw new Error(
					`resilientFetch: unhandled outcome ${JSON.stringify(_exhaustive)}`,
				);
			}
		}
	}
